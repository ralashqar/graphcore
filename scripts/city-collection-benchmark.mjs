...
const browser=await chromium.launch({channel:'msedge',headless:true,args:['--use-angle=d3d11']}),results=process.env.CITY_BENCH_COUNT&&existsSync('output/city-collection-benchmark.json')?JSON.parse(readFileSync('output/city-collection-benchmark.json','utf8')).filter(r=>r.count!==Number(process.env.CITY_BENCH_COUNT)):[];
try{for(const count of (process.env.CITY_BENCH_COUNT?[Number(process.env.CITY_BENCH_COUNT)]:[72,400]))for(const kit of ['legacy','collection']){
 ...
 await page.evaluate(async({kit,count})=>{
  ...
  for(const [i,p] of world.plots.slice(0,6).entries()){
   const d=collectionPreset(initialLandDraft(p),16+i,p.size);
   if(kit==='legacy'){
    const s=d.sculpt.studio;s.catalogue='synarc-kit-3';s.defaults.window='window-sash';delete s.roofDetails;for(const part of Object.values(s.parts))if(part.window)part.window='window-sash';
 ...
 results.push({count,kit,localPlots:6,...data,errors});writeFileSync('output/city-collection-benchmark.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results.at(-1)));await page.close();
