...
try {
 ...
 const p=r.request().postDataJSON(),url=r.request().url();const reply=json=>r.fulfill({json});
 if(url.endsWith('city-command')&&p.action==='save') {business.draft=p.profile;business.preview=p.profile;business.draft_version++;saved++;return reply({ok:true});}
 ...
 await page.getByRole('region',{name:'Live 3D building designer'}).waitFor();
 await page.locator('.city-design-canvas canvas').waitFor();await page.waitForTimeout(800);
 ...
 await page.getByLabel('Facade style',{exact:true}).selectOption('piers');
 await page.getByLabel('Tile styling',{exact:true}).selectOption('slate');
 await page.getByLabel('Building orientation',{exact:true}).selectOption('1');
 assert.equal(await page.locator('.city-design-canvas').getAttribute('data-blueprint'),'l-shape');
 assert.equal(await page.locator('.city-design-canvas').getAttribute('data-floors'),'5');
 const after=await page.locator('.city-design-canvas').screenshot();assert.notDeepEqual(before,after);
 await page.getByRole('region',{name:'Live 3D building designer'}).evaluate(el=>el.scrollIntoView({block:'start'}));
 await page.screenshot({path:'output/playwright/city-design-desktop.png'});
 await page.getByRole('button',{name:'Blueprint view',exact:true}).click();
 await page.getByRole('img',{name:'l-shape footprint blueprint'}).waitFor();
 await page.screenshot({path:'output/playwright/city-design-blueprint.png'});
 ...
 assert.equal(saved,1);assert.equal(business.draft.buildingDesign.blueprint,'l-shape');assert.equal(business.draft.buildingArt,'');
 await page.reload();await page.locator('.city-design-canvas[data-floors="5"]').waitFor();
 await page.setViewportSize({width:390,height:844});
 await page.getByRole('region',{name:'Live 3D building designer'}).evaluate(el=>el.scrollIntoView({block:'start'}));
 assert.ok(await page.locator('.city-design-canvas canvas').isVisible());
 assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
 await page.screenshot({path:'output/playwright/city-design-mobile.png'});
 business.published=business.draft;await page.setViewportSize({width:1440,height:1000});await page.goto(`${origin}/city`);
 await page.waitForFunction(()=>document.querySelector('canvas')?.dataset.cityCamera);await page.waitForTimeout(1200);
 await page.screenshot({path:'output/playwright/city-design-map.png'});
 await page.mouse.click(885,553);await page.getByRole('complementary',{name:'Fieldwork property',exact:true}).waitFor();
 assert.deepEqual(errors,[]);console.log('3D designer: live presets, geometry changes, blueprint, save/reload, mobile and city rendering passed (mock API).');
