...
await page.goto(`${process.env.CITY_TEST_ORIGIN||'http://127.0.0.1:5188'}/city${count===72?'?demo=1'+(process.env.CITY_BENCH_LAND==='0'?'&cityBuild=0':''):''}`);
...
if(process.env.CITY_BENCH_MODE==='walking'){await page.waitForFunction(()=>JSON.parse(document.querySelector('canvas')?.dataset.cityExploration||'{}').character==='ready');await page.keyboard.press('e');await page.getByRole('region',{name:'Walking controls'}).waitFor();await page.waitForTimeout(3000);}
...
await page.keyboard.down('w');clearInterval(progress);await page.waitForTimeout(4000);
if(process.env.CITY_BENCH_MODE==='walking'){await page.waitForFunction(()=>JSON.parse(document.querySelector('canvas')?.dataset.cityExploration||'{}').character==='ready');await page.keyboard.press('e');await page.getByRole('region',{name:'Walking controls'}).waitFor();await page.waitForTimeout(3000);}await page.keyboard.down('a');await page.waitForTimeout(1200);await page.keyboard.up('a');await page.waitForTimeout(2500);await page.keyboard.down(' ');await page.waitForTimeout(700);await page.keyboard.up(' ');await page.keyboard.up('w');
const data=await page.evaluate(()=>{window.driveMeasure=false;const a=window.driveFrames.slice(2).sort((a,b)=>a-b);return {p95:a[Math.floor(a.length*.95)],p50:a[Math.floor(a.length*.5)],frames:a.length,stats:document.querySelector('canvas').dataset.cityRenderStats,physics:document.querySelector('canvas').dataset.cityDrivePerformance,backend:document.querySelector('canvas').dataset.cityBackend};});results.push({count,mode:process.env.CITY_BENCH_MODE||'driving',...data,errors});console.log(results.at(-1));await writeFile(`output/city-drive-${process.env.CITY_BENCH_LABEL||'after'}.json`,JSON.stringify(results,null,2));await page.close();}
