...
try {
 ...
 await page.getByRole('button',{name:'AI artwork',exact:true}).click();
 ...
 await page.reload();await page.getByRole('button',{name:'AI artwork',exact:true}).click();await page.getByAltText('Fieldwork building preview',{exact:true}).waitFor();await page.setViewportSize({width:390,height:844});await page.getByRole('region',{name:'Building artwork'}).evaluate(el=>el.scrollIntoView({block:'start'}));
