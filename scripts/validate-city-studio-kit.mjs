...
assert.equal(manifest.parts.length,version===2?64:version===3?72:version===4?105:141);assert.equal(new Set(manifest.parts.map(p=>p.id)).size,manifest.parts.length);
...
for(const part of catalogue.parts){
 ...
 assert.ok(doc.nodes.some(n=>n.name===(version>2?`v${version}/${part.id}`:part.id)),`${part.id} export missing`);
...
if(version>=4){
 for(const mesh of doc.meshes)for(const primitive of mesh.primitives)assert.ok(primitive.attributes.TEXCOORD_0!==undefined,'NYC geometry must retain UVs');
 const presets=json(`${base}/presets.json`);assert.equal(presets.length,version===4?6:24);
 for(const preset of presets){assert.deepEqual(preset.studio.inactive,[]);for(const ext of ['png','glb'])assert.ok(existsSync(new URL(`${base}/presets/${preset.id}.${ext}`,root)));}
}
