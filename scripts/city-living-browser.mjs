...
try{
 ...
 await context.route('**/functions/v1/city-*',async route=>{
  ...
  switch(i.action){
   case 'snapshot':return reply({revision:1,capacity:400,total:1,properties:[property],events:[],discoveryEnabled:true,campusEnabled:true,dealsEnabled:true,customerDiscoveryEnabled:true,storefrontsEnabled:true,activityEnabled:true});
   case 'customer_city_state':return reply({states:i.ids.includes(businessId)?[{businessId,kind:'ending',band:'very_busy',primary:item,windowStart:new Date(Date.now()-172800000).toISOString(),measuredAt:new Date().toISOString(),expiresAt:new Date(Date.now()+300000).toISOString(),version:1}]:[]});
 ...
 await page.goto(origin+'/city/business/campus-studio');
 await page.getByRole('button',{name:'Follow business',exact:true}).click();
 await page.getByRole('button',{name:'Following',exact:true}).waitFor();
 assert.ok(actions.some(a=>a.action==='discovery_follow'&&a.enabled));
 await page.getByRole('button',{name:'Following',exact:true}).click();
 await page.getByRole('button',{name:'Follow business',exact:true}).waitFor();
 await page.getByRole('button',{name:'New Drops',exact:true}).click();
 await page.getByRole('button',{name:/Launch.*Campus Studio.*Creator launch/}).first().click();
 await page.waitForURL(url=>url.pathname==='/city/launches/creator-launch');
 await page.getByRole('complementary',{name:'Business launch'}).waitFor();
 assert.equal(actions.filter(a=>a.action==='customer_reminder').length,0);
 await page.getByRole('button',{name:'Remind me in City',exact:true}).click();
 await page.getByRole('button',{name:'Cancel in-app reminder',exact:true}).waitFor();
 await page.getByRole('button',{name:'What’s new · 1',exact:true}).click();
 await page.getByRole('dialog',{name:'Following / What’s New'}).waitFor();
 await page.getByRole('button',{name:'Mark read',exact:true}).click();
 await page.getByRole('button',{name:'What’s new',exact:true}).waitFor();
 await page.keyboard.press('Escape');
 await page.reload();
 await page.getByRole('button',{name:'Cancel in-app reminder',exact:true}).waitFor();
 await page.getByRole('button',{name:'Cancel in-app reminder',exact:true}).click();
 await page.getByRole('button',{name:'Remind me in City',exact:true}).waitFor();
 await page.screenshot({path:'output/playwright/city-living-desktop.png'});
 await page.setViewportSize({width:390,height:844});
 assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'living mobile has no overflow');
 await page.getByRole('button',{name:'Minimise launch',exact:true}).click();
 await page.getByRole('button',{name:'Expand launch',exact:true}).waitFor();
 await page.waitForTimeout(1000);
 await page.screenshot({path:'output/playwright/city-living-mobile.png'});
 await page.emulateMedia({reducedMotion:'reduce'});await page.reload();
 await page.getByRole('complementary',{name:'Business launch'}).waitFor();
 assert.deepEqual(errors,[]);
 console.log(JSON.stringify({passed:true,usefulContentMs,checks:'guest discovery and authenticated claims with purchases off, follows, launch fly-to, explicit reminder, inbox read, reload/cancel, mobile sheet, reduced motion, runtime'}));
