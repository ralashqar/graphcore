...
const browser=await chromium.launch({channel:'msedge',headless:true,args:['--use-angle=d3d11']}),results=process.env.CITY_BENCH_COUNT&&existsSync('output/city-variation-benchmark.json')?JSON.parse(readFileSync('output/city-variation-benchmark.json','utf8')).filter(r=>r.count!==Number(process.env.CITY_BENCH_COUNT)):[];
try{for(const count of (process.env.CITY_BENCH_COUNT?[Number(process.env.CITY_BENCH_COUNT)]:[72,400]))for(const kit of ['legacy','variation']){
 ...
 await page.evaluate(async({kit,count})=>{
  const {initialLandDraft,LAND_OWNER}=await import('/src/domain/cityLand.ts'),{collectionPreset}=await import('/src/domain/cityCollectionPresets.ts');
  const key=Object.keys(localStorage).find(k=>k.startsWith('city-land-v1-')),world=JSON.parse(localStorage.getItem(key));
  if(count>=72){
   const {createModularDesign}=await import('/src/domain/cityModularBuilding.ts');
   ...
   world.occupied=cityPlots(500).filter(p=>!local.has(`${p.x}:${p.z}`)).slice(0,count).map((p,i)=>({...p,id:`fixture-${i}`,slug:`fixture-${i}`,rank:i+1,tier:buildingTier(10000),landValue:10000,saves:0,claims:0,profile:{...emptyCityProfile(),name:`Fixture ${i+1}`,buildingDesign:kit==='variation'?createModularDesign(newDesign(`drive-${i%6}`),16+i%8):newDesign(`drive-${i%6}`),website:'https://example.com'}}));
 ...
 if(kit==='variation')await page.waitForFunction(()=>['ready','proxy'].includes(document.querySelector('canvas')?.dataset.cityStudioKit),null,{timeout:90000});
 ...
 const data=await page.evaluate(()=>{window.nycMeasuring=false;const frames=window.nycFrames.slice(2).sort((a,b)=>a-b),c=document.querySelector('canvas');return {mean:frames.reduce((a,b)=>a+b,0)/frames.length,p99:frames[Math.floor(frames.length*.99)],max:frames.at(-1),over50ms:frames.filter(n=>n>50).length,sampledMs:frames.reduce((a,b)=>a+b,0),p95:frames[Math.floor(frames.length*.95)],median:frames[Math.floor(frames.length*.5)],frames:frames.length,stats:JSON.parse(c.dataset.cityRenderStats||'{}'),backend:c.dataset.cityBackend,prepared:c.dataset.cityPreparedBuildings,resident:document.querySelector('[data-city-resident-count]')?.getAttribute('data-city-resident-count')};});
 results.push({count,kit,localPlots:0,...data,errors});writeFileSync('output/city-variation-benchmark.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results.at(-1)));await page.close();
