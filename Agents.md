...
- `cinematic_episode_from_sequence` and `cinematic_trailer` story/movie requests default to `cinematicPipelineVersion = "v3_script_storyboards"`. V3 is built on the existing Output Workflow tables rather than a separate cinematic-project table: `world_context -> skill_context -> cinematic_entities -> cinematic_v3_reference_select -> cinematic_v3_screenplay_author -> cinematic_v3_shot_parse -> cinematic_v3_storyboard_group_plan -> cinematic_v3_dynamic_storyboard_fanout`. It deliberately skips V2 scene compile, layout plan, separate shot-plan, per-shot asset-pack, per-shot keyframe, keyframe QA, and per-shot video nodes. The screenplay author remains creative-only and returns readable Markdown screenplay/treatment text. The V3 shot parser consumes that screenplay plus selected references and emits strict shot JSON directly: timed shots with action, dialogue, caption, visible/speaker refs, location/prop refs, camera, lighting, mood, `performanceBeats[]`, `storyboardPanelPrompt`, `videoDirection`, total editorial duration, audio plan, timeline notes, and diagnostics. The deterministic storyboard group planner splits shots into video-sized storyboard blocks of 15 seconds or less, with a maximum of 9 panels per sheet and fixed 1x1, 2x2, or 3x3 crop grids chosen from the actual shot count in that block. A slow 15-second-or-less block may legitimately have only 1-2 panels; sheets with fewer shots than grid cells must leave row-major blank placeholders and forbid masonry layouts, staggered rows, merged panels, unequal cells, diagonal dividers, and extra panels so panel extraction can crop reliably from each group's stored `rows`, `columns`, and `panelCount`. The V3 dynamic fanout materializes one branch per storyboard/video block: prompt, GPT Image 2 high-quality WebP sheet, deterministic panel extraction, optional approval-gated storyboard-group video prompt/video using that block's duration, timeline assembly, and artifact registration. Initial V3 cinematic generation stops after screenplay, shot JSON, storyboard sheets, extracted panels, and timeline/animatic metadata; storyboard-group video nodes remain blocked/skipped unless `cinematicVideoApproved = true` or a manual approved rerun targets them. V2 remains available with `cinematicPipelineVersion = "v2_shot_orchestration"` for rollback and Director Notes tooling, and V1 remains available with `cinematicPipelineVersion = "v1_take_blocks"`.
