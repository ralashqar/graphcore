...
- `start-output-request` is the prompt-first entry point. It classifies whether a prompt is output generation, world mutation, answer-only, or ambiguous; output prompts select an approved preset/template, bind mentioned world entities/sequence units, persist a workflow and queued run, and link both back to the request. Non-output/ambiguous prompts remain request rows awaiting confirmation rather than mutating world canon.
