...
- Durable queued visual jobs for `world_entity_icon_grid`, `brand_atlas`, `screen_mockup`, `entity_reference_sheet`, `character_sheet`, `wiki_visual`, and `app_screen_mockup`
...
- First-run streamed world seeding queues `entity_reference_sheet` jobs idempotently as each eligible `upsert_entity` lands. These jobs are the initial entity art path in the live Wiki surface; the seed worker must not auto-queue the legacy `world_entity_icon_grid` batch for initial worlds.
...
- Wiki gap tooling still includes manual batch entity icon generation as a recovery/tooling action. The Wiki can start a protected `start-world-entity-icon-batch` request that selects up to 16 active world entities missing both `thumbnailAssetKey` and linked definition `iconAssetKey`, then enqueues a generic `visual_generation_jobs.kind = "world_entity_icon_grid"` job. This legacy icon-grid path must remain separate from first-run world seeding, where `entity_reference_sheet` is the preferred production art path.
