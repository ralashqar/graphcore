...
- Catalog-ranked prompt intent routing must never surface `answer_only` classifier rationale as a client error. Answer-only critique, evaluation, and “where should this be fleshed out?” prompts should still create a world-prompt feed turn with `sourceContext.promptMode = "ask"` so the backend can return an advisory answer plus clickable suggestions without mutating canon by default. Only genuinely ambiguous or confirmation-required non-answer intents should block before starting a turn.
...
- Resetting a project world clears generated wiki metadata and the draft `projectContext` so a deleted app/game/story world cannot leak stale project-type strategy into the next first-run seed. After art-style confirmation, `continue-world-seed-generation` must immediately persist the transient inferred project context to the draft and job snapshot before the streamed seed finishes; the completed context with `onboardingCompletedAt` is written again at terminal seed completion.
...
- V3 screenplay/treatment authoring before technical parsing, focused on scene objective, emotional arc, dialogue/action beats, and shot-readable prose
- Sequence-unit screenplay animatics launch a dedicated Cinematic V3 master flow with `sequenceAnimaticMode = "master_script_only"` from Wiki chapter pages. The master request is marked `metadata.sequenceAnimaticRole = "master"` and runs only through world context, references, screenplay authoring, shot-break planning, parse-block fanout, and a durable `sequence_animatic_manifest` artifact. It must not generate storyboard sheets, panel extracts, video prompts, or videos.
- Sequence-unit storyboard block video generation is gated behind completed block prep. Wiki block UI should only expose `Generate video` when the child block has a storyboard sheet, extracted panels for every shot, and a completed `video_prompt` node with persisted prompt text. The worker must reject Cinematic V3 storyboard video nodes that lack an upstream video prompt or storyboard-sheet reference instead of falling back to the request prompt and submitting a malformed MUAPI/Seedance job. V3 storyboard sheets may be `planningOnly` authoring assets, but if they are marked `usedAsVideoReference` they must still be passed to Seedance/MUAPI as the direct storyboard reference.
- Sequence animatic storyboard blocks are saved as child `output_requests` with `parent_request_id` pointing to the master and `metadata.sequenceAnimaticRole = "storyboard_block"`. Each child workflow owns one block: `block_input -> storyboard_prompt -> storyboard_sheet -> panel_extract -> video_prompt`, with a manual-only video node. Child workflows are created in `awaiting_confirmation` / `readyToRun` state and are not run until the sequence page's per-block Generate Storyboard action starts that child workflow.
- Sequence animatic per-shot videos are lazy nested children under a storyboard block request with `metadata.sequenceAnimaticRole = "shot_video"`. They are created only when the user asks for a shot take, use a mini graph `shot_input -> shot_video_prompt -> shot_video`, and must use the cropped extracted panel as `@Image1` keyframe reference. Shot video prompts are scoped to one parsed shot, include only visible shot character/location/prop art references, keep offscreen speakers in the dialogue/voice guide without attaching their visual art, infer natural editorial timing from the shot's action, dialogue, camera movement, and settle time instead of blindly reusing rough screenplay marker timing, and use that inferred duration as the provider duration parameter and prompt shot range. Shot prompts store compact `directedControls` for camera motion, subject motion, focus, framing lock, visibility, motion intensity, performance, and voice. Voice guidance should carry available character context such as age/gender cue, accent, pitch, register, pace, and delivery quality. Shot-video native audio is intentionally narrow: scripted dialogue plus direct diegetic sound effects only, with music, score, audio beds, room tone, crowd wash, and general ambience explicitly forbidden. Provider prompts must prefer those concise controls and a short shot line over verbose cinematic prose, and must not duplicate duration in the opening line. Videos register with role `sequence_animatic_shot_video`.
- V3 script-to-shot JSON parsing for story cinematics. The parser converts the authored screenplay directly into timed shots with action, dialogue/caption, camera, lighting, mood, visible/speaker refs, storyboard panel prompts, video direction, and `performanceBeats[]` without mutating world canon.
- Legacy V2 screenplay, scene-state, layout, and shot-orchestration support remains available when explicitly requested for rollback or Director Notes tooling.
...
- Shared TypeScript node registry in `src/domain/outputWorkflow.ts` validates approved node types for frontend previews, Edge Functions, and workers
  ...
  - Retry behavior is hash-based: rerunning a failed workflow against the same persisted workflow/input skips completed nodes whose `inputHash` still matches, retries failed nodes, and unblocks downstream nodes. Longform prose nodes persist provider request IDs so a stale worker can resume provider polling instead of creating duplicate OpenAI requests. Dynamic cinematic fanout outputs are only treated as fresh graph expansion signals when the fanout node actually executes in that scheduler pass; cached/skipped fanout outputs must not cause repeated expansion passes or a false "did not settle" workflow failure.
  ...
  - Cinematics V3 authoring timelines are durable timeline artifacts, not transient graph state. The script-marked V3 dynamic graph must materialize `cinematic_v3_timeline_assemble` plus the final `artifact` node, run storyboard sheet extraction into `cinematic_v3_storyboard_panel` artifacts, persist the merged `shotPlan` / `shot_plan` on the timeline node output, and register a `cinematic_v3_authoring_timeline` artifact containing the timeline, shot plan, panel asset keys, and video-prompt summaries. The timeline UI may open from compact Output rows without bulk-loading every node output, so it must be able to reconstruct the scrub view from workflow metadata plus `output_artifacts`, hydrating the selected timeline node only as fallback.
  ...
  - Output feed/status views use a compact `output_request_status_projections` table and the `get-output-feed` Edge Function instead of repeatedly hydrating workflow nodes, edges, runs, artifacts, and run steps from the browser. Projection rows track request/run status, progress counts, active node labels, parallel active-node metadata, active provider status/request IDs/elapsed queue time, artifact/asset keys, graph/timeline revisions, and terminal state; full DAG and selected-node payloads remain behind `get-output-workflow-graph` and on-demand node hydration. The first output-feed page also explicitly supplements recent `wiki_sequence_unit` sequence-animatic master requests and their child storyboard-block requests so chapter pages can rediscover linked animatics after a full reload even when child block rows or unrelated outputs push the master outside the projection page. Realtime output inbox signals should subscribe to `output_requests` plus `output_request_status_projections` only, treating events as coalesced refresh signals rather than full state. While any output request is queued/planning/running, the app shell keeps a lightweight projection watcher alive even when the graph inspector is closed, with realtime wake-ups plus watchdog polling; opening Graph is for detailed inspection only and must not be required for compact progress to advance.
  - Output image and cinematic reference selection must keep visual variant text and image inputs aligned. When an output prompt selects an entity visual variant, the worker stores the selected variant asset as that entity's `primaryAssetKey`, sends that primary reference before any fallback/default references to provider image/storyboard nodes, and persists `output_requests.metadata.outputReferenceSelection` as soon as the reference-selection node completes. Compact Outputs rows read that snapshot before final artifacts exist, so they should not show a default sheet and later swap to the selected variant.
  - Output image and cinematic reference selection must keep visual variant text and image inputs aligned. When an output prompt selects an entity visual variant, the worker stores the selected variant asset as that entity's `primaryAssetKey`, sends that primary reference before any fallback/default references to provider image/storyboard nodes, and persists `output_requests.metadata.outputReferenceSelection` as soon as the reference-selection node completes. Compact Outputs rows read that snapshot before final artifacts exist, so they should not show a default sheet and later swap to the selected variant.
  - Fal-backed GPT Image 2 jobs use hybrid webhooks for both durable `visual_generation_jobs` and output workflow `image_generation` steps. Submissions attach the shared `fal-webhook` callback when configured, and the webhook only records provider result metadata such as `webhookStatus`, `falWebhookImageUrl`, provider error details, and received timestamps on the matching job/step. The Fly worker remains the only finalization authority: it consumes that metadata as a wake-up/result cache, downloads the provider image, uploads to Supabase Storage, upserts `project_assets`, registers output artifacts, updates entity/wiki/output bindings, and completes/fails the job or step. Polling remains the fallback when no webhook arrives or the webhook lacks a usable image URL. `FAL_WEBHOOK_REQUIRE_SIGNATURE=true` enables strict production signature enforcement; direct OpenAI image jobs do not use Fal webhooks.
  ...
  - Seedance prompts use a shared reference-manifest contract. The worker builds `seedanceReferenceManifest` entries from the actual submitted provider order and only refers to attached references as `@ImageN`, `@VideoN`, or `@AudioN`. V3 storyboard-block videos prefer storyboard sheet first, then selected character/location/prop variant references, then optional video/audio refs; V2 keyframe videos preserve the shot keyframe as `@Image1`. If a provider fallback drops references, the provider prompt is rebuilt from the reduced manifest so stale `@ImageN` labels do not remain. V3 Seedance prompts use a compact structure: intent line without duplicated duration, exact reference legend, storyboard/keyframe instruction when relevant, timestamped shot call sheet, directed controls, identity/speaker guide, and one concise production-board artifact ban. MUAPI VIP `seedance-2-vip-omni-reference` rejects prompts over 4000 characters, so the worker must compact generated provider prompts to a safe budget before submission, shortening identity guides and dropping lower-priority movement/constraint prose before truncating. Laban movement logic is included only for high-physicality action such as fights, martial arts, chases, impacts, aerial motion, or weapon choreography.
...
- Prompt-first Output Studio requests can route to approved templates only. V1 output kinds include `concept_art_image`, `poster_image`, `story_bible_from_world`, `world_reference_document`, `lore_guide`, `character_dossier_pack`, `short_story`, `narrative_chapter_or_ebook`, `ebook_from_world`, and `comic_issue_from_sequence`. The prompt planner returns a structured approved-template plan with intent, output kind, confidence, selected world scope, document mode, selected entities/sequences, planned sections, visual reference policy, and confirmation state; it must ask for confirmation on ambiguous/low-confidence prompts instead of silently falling back to an ebook. Prompt-first image requests run an LLM world-scope resolver before the workflow is persisted, using the user prompt plus a compact world-entity/sequence catalog to choose only the explicitly requested or visually required entities. Deterministic name matching is a safety fallback only; image workflows must not broaden empty selections to first-N world entities, chapter casts, or unrelated relationship neighbors. Concept/poster requests use a compact `world_context -> skill_context -> visual_prompt + image_references -> image_generation` workflow backed by the existing Fal GPT Image 2 image node. When a prompt binds world entities such as characters, places, or items, the workflow must build an `asset_pack` from those entities and pass available thumbnail/image project assets into the image node `references` input so Fal uses the edit/reference route instead of relying on text-only descriptions.
  - Prompt-first still-image outputs are variant-aware. `start-output-request` loads `world_entity_visual_variants` into the scope resolver catalog so prompts like "Suri in samurai outfit" or "inside the Pact Chamber" can select the parent character/location even when the named phrase is a visual variant, not a canonical entity. The worker refreshes variants again during `world_context_query`, and `image_reference_selector` chooses exactly one primary reference per selected entity: the default reference sheet unless the prompt matches a usable visual variant such as a wardrobe/look variant or a shot-location variant. A variant is usable when it has an asset key and is not pending/running/failed/cancelled, even if the status text is missing or nonstandard. The asset pack stores `selectedReferenceVariantKey`, label, summary, type, selected asset key, and diagnostics such as `variant_pending`, `variant_not_found`, or `missing_reference`; it must not pass every variant for the same entity into one image-generation node because conflicting references dilute identity. Generated image artifacts record the selected variant keys/assets for replay and debugging, and Wiki-style output rows must read artifact metadata when run steps are status-only. Cinematic reference selection uses the same variant metadata: multi-word shot-location matches such as "leader's chamber of Whistlewick" must keep the parent location reference selected, treat the shot-location variant as that location's primary visual reference, and reject unrelated props/items that only match generic words like "chamber".
...
- `cinematic_episode_from_sequence` and `cinematic_trailer` story/movie requests default to `cinematicPipelineVersion = "v3_script_storyboards"`. V3 is built on the existing Output Workflow tables rather than a separate cinematic-project table: `world_context -> skill_context -> cinematic_entities -> cinematic_v3_reference_select -> cinematic_v3_screenplay_author -> cinematic_v3_shot_break_plan -> cinematic_v3_dynamic_shot_parse_fanout`, then the dynamic parse fanout materializes each storyboard block directly: `cinematic_v3_shot_parse_group_###` -> `cinematic_v3_storyboard_group_###_prompt` -> `cinematic_v3_storyboard_group_###_sheet` -> `cinematic_v3_storyboard_group_###_panel_extract` -> `cinematic_v3_storyboard_group_###_video_prompt`, plus manual-only video nodes and the shared timeline/artifact nodes. It deliberately skips V2 scene compile, layout plan, per-shot asset-pack, per-shot keyframe, keyframe QA, and per-shot video nodes. The V3 screenplay author writes creative Markdown screenplay/treatment text again, but must place lightweight shot anchors such as `<!-- SHOT 001: threshold reveal | ~3s -->` before intended storyboard/video beats and store `metadata.scriptContract = "screenplay_with_shot_markers_v1"`. The deterministic shot-break planner extracts those anchors, titles, rough durations, source text spans, and groups of 15 seconds or less with up to 9 shots; missing anchors fall back to paragraph-derived breaks with diagnostics. The dynamic parse fanout runs one background/resumable OpenAI parse node per storyboard/video block so long scripts do not require one foreground 120-second parse call. There is no required graph-level shot-plan merge in the default V3 path: each storyboard block parse feeds its own prompt, sheet, extraction, and video-prompt branch. Timeline assembly collects the block-level shot plans only to present a continuous authoring timeline. The resulting shot JSON carries timed shots with action, dialogue, caption, visible/speaker refs, location/prop refs, camera, lighting, mood, `performanceBeats[]`, `storyboardPanelPrompt`, `videoDirection`, total editorial duration, audio plan, timeline notes, and diagnostics. The deterministic storyboard group planner splits or preserves groups into video-sized storyboard blocks of 15 seconds or less, with a maximum of 9 panels per sheet and V3 crop grids chosen from the actual shot count in that block: 1x1 for one panel, 1x2 for two panels, 2x2 for three to four panels, 2x3 for five to six panels, and 3x3 for seven to nine panels. A slow 15-second-or-less block may legitimately have only 1-2 panels; sheets with fewer shots than grid cells must leave row-major blank placeholders and forbid masonry layouts, staggered rows, merged panels, unequal cells, diagonal dividers, and extra panels so panel extraction can crop reliably from each group's stored `rows`, `columns`, and `panelCount`. The V3 parse fanout materializes one storyboard branch per storyboard/video block immediately after the block parser: prompt, GPT Image 2 high-quality WebP sheet, deterministic panel extraction, optional approval-gated storyboard-group video prompt/video using that block's duration, timeline assembly, and artifact registration. V3 storyboard-group video prompts must include a local timestamped call sheet derived from each shot's `editorialDurationSeconds`, plus a character identity/speaker guide with visual descriptions and `metadata.voice` descriptions, so Seedance receives explicit per-shot timing and clear speaker identity rather than only shot order. Initial V3 cinematic generation stops after screenplay, shot JSON, storyboard sheets, extracted panels, video prompts, and an authoring/timeline artifact; storyboard-group video nodes are materialized but `manualOnly`, their edges into timeline assembly are optional, and they are omitted from default scheduler execution unless `cinematicVideoApproved = true` or a targeted manual rerun includes them. V3 dynamic rematerialization must preserve existing same-hash node outputs, hashes, `metadata.outputPreview`, and execution cache for parse groups, storyboard sheets, panel extract, video prompt, timeline, and artifact nodes; if a same-hash dynamic node row has been wiped but the latest run step still contains durable outputs, the materializer repairs the node from that run-step output instead of leaving the graph preview blank. If a provider-backed node uploaded an `output_artifacts` / `project_assets` record but the worker was interrupted before marking the node/step complete, the worker must recover node outputs from the artifact and mark the run step completed before scheduling dependents. Graph refresh/reopen remains status-only at scale but recovers visible previews from `output_artifacts`, `metadata.outputPreview.assetKeys`, and latest run-step preview metadata, while selected-node hydration falls back to latest run-step outputs and artifact-derived previews when node rows were wiped. Fal storyboard sheet progress records `providerSubmittedAt`, elapsed provider time, and stale request restart metadata so long-running provider jobs can be diagnosed and retried without losing graph state. V2 remains available with `cinematicPipelineVersion = "v2_shot_orchestration"` for rollback and Director Notes tooling, and V1 remains available with `cinematicPipelineVersion = "v1_take_blocks"`.
- Cinematic V3 shot parsing must repair structured visual references after the LLM parse and before validation/artifact creation. If a selected entity or selected location variant is clearly mentioned in shot text, dialogue speaker labels, storyboard panel prompts, or video direction, add the canonical character/location/prop key to the shot (`visibleCharacterRefIds`, `speakerRefIds`, `locationRefId`, or `propRefIds`) rather than leaving it as prose only. This keeps panel reference icons, asset packs, storyboard prompts, and later Seedance prompts aligned with what is actually visible on screen.
  - Sequence-unit Wiki pages launch durable screenplay animatics through a trusted UI action, not prompt classification. The client calls `start-output-request` with `sourceSurface = "wiki_sequence_unit"`, `outputKindOverride = "cinematic_episode"`, `selectedSequenceUnitKeys = [sequenceKey]`, `cinematicPipelineVersion = "v3_script_storyboards"`, and `sequenceAnimaticMode = "master_script_only"`. After the master manifest exists, `ensure-sequence-animatic-block-workflows` creates one saved child request/workflow per storyboard block without running them. The sequence page assembles a partial/live animatic view from the master manifest plus child runs/artifacts: screenplay text, parsed shots, empty panel slots, extracted panel assets, block statuses, block graph links, and manual video entry points. Removing the master request must delete all child block graphs/artifacts/assets through output cleanup; deleting a child removes only that block.
  - Cinematic V3 storyboard layout policy is `1 -> 1x1`, `2 -> 1x2`, `3-4 -> 2x2`, `5-6 -> 2x3`, and `7-9 -> 3x3`. Extraction must use the persisted rows, columns, and panel count for the block; historical sheets keep their old layout until regenerated.
  ...
  - V2 final video estimates treat Seedance units as generated seconds. MUAPI `seedance-2-vip-omni-reference` uses a conservative `$0.30/s` estimate until a more exact configured rate is available; legacy non-VIP `seedance-2.0-omni-reference` / `sd-2-omni-reference` aliases and the `sd-2-vip-omni-reference` shorthand are normalized to the canonical VIP provider endpoint before submission. Fal Seedance estimates keep the existing model-specific per-second snapshots. Store the estimate on run/request metadata when approval starts so users can see final production cost separately from preview animatic text/image costs.
...
**Integration**:
- Public Edge Functions: `start-output-request`, `get-output-request-status`, `cancel-output-request`, `delete-output-request`, `repair-output-workflow-state`, `plan-output-workflow`, `start-output-workflow`, `start-output-workflow-run`, `ensure-sequence-animatic-block-workflows`, `get-output-workflow-status`, `cancel-output-workflow-run`, `get-output-workflow-graph`, `get-output-artifact`, `update-output-workflow-node`, `upgrade-output-workflow-preset`, `preview-output-cinematic-director-note`, and `apply-output-cinematic-director-patch`
- `start-output-request` is the prompt-first entry point. It normally runs the structured output planner to classify whether a prompt is output generation, world mutation, answer-only, or ambiguous; output prompts select an approved preset/template, bind mentioned world entities/sequence units, persist a workflow and queued run, and link both back to the request. Trusted product UI actions may pass `outputKindOverride` to bypass classifier ambiguity while still using the same planner, scope resolver, workflow creation, run queueing, and metadata persistence path; this is intended for explicit buttons such as sequence-unit screenplay animatics, not free-form user prompt routing. For concept/poster image requests, it also runs an OpenAI Responses JSON-schema scope resolver before workflow creation so prompts such as “Ilya saluting to Anya” bind only the Ilya and Anya entity keys and their available visual assets. The resolver uses `OUTPUT_REQUEST_PLANNER_MODEL` when configured, falls back to `OUTPUT_WORKFLOW_TEXT_MODEL`, and otherwise defaults to `gpt-5.4`; it stores scope provenance in request/workflow/run metadata. Non-output/ambiguous/low-confidence prompts remain request rows awaiting confirmation rather than mutating world canon or defaulting to the wrong artifact type.
