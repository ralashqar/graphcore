...
- Low-level malformed streamed fragments are logged to Edge Function logs rather than written as user-facing assistant notes, so onboarding progress remains a clean creation log. Contract-valid graph ops, applied ops, deferred relationships, and terminal failures still emit normal prompt events.
