...
- Shared TypeScript node registry in `src/domain/outputWorkflow.ts` validates approved node types for frontend previews, Edge Functions, and workers
  ...
  - `video_generation` nodes now have a worker-side Fal queue executor path for Seedance-style text-to-video jobs. Video outputs are uploaded to `project-assets`, registered as `output_artifacts.kind = "video"`, and accounted through the same usage summary path as text and image nodes.
...
- `video_generation`: runs worker-side Fal Seedance 2 reference-to-video nodes, with storyboard/image references ordered as `@Image1`, 4-15 second duration enforcement, 480p/720p/1080p resolution controls, optional native audio, usage estimates, and generated video assets/artifacts
...
- `cinematic_episode_from_sequence`, `cinematic_trailer`, and `ugc_episode`: one selected `sequence_unit` -> world context -> cinematic/UGC/Seedance skills -> cinematic entity asset pack -> fixed block plan -> one timestamped block script per video unit -> deterministic storyboard prompt -> GPT Image 2 low-quality WebP storyboard grid -> deterministic Seedance prompt -> Fal Seedance 2 reference-to-video block -> worker-side ffmpeg stitch -> final video artifact.
  - Cinematic V1 defaults to 3 video blocks, 8 seconds each, `16:9`, `720p`, and `generateAudio: true`; UGC-oriented prompts default to `9:16`. Block fan-out is capped at 6 and each block is capped to Seedance's 4-15 second duration range.
  - Storyboards use `openai/gpt-image-2` through Fal at low quality, WebP output, and `2048x2048` grid images. The storyboard image is the first Seedance reference (`@Image1`); entity, environment, and prop references from the asset pack follow as continuity anchors.
