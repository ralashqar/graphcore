...
- `story_bible_from_world`: world context query -> reference skills -> section plan -> parallel approved reference-section nodes -> reference assembly -> document render -> output artifact.
  ...
  - Reference PDFs use worker-side document rendering in reference mode with handbook-style layout and companion Markdown artifacts, while preserving the same workflow/run/artifact provenance contract as ebooks.
