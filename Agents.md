...
- Uses an incremental manifest-and-work-item executor for initial seed generation and broad/sequence-heavy prompt turns. The agent first plans small ordered work items, then generates and applies each item through the normal graph mutation path so entity, sequence-unit, and relationship progress is visible before the full turn completes.
...
- Normal follow-up `world-prompt` turns remain compact modification turns. Expanded scope/caps apply only when `initialSeedMode` is `generate_skeleton`, so first generation can create the full starter graph while later prompts stay focused.
...
- Incremental world-prompt execution is selected for initial skeleton generation, broad seed-world requests, large source contexts, sequence-heavy prompts, and high explicit entity counts. Small follow-up edits keep the compact single-plan path for lower latency.
