...
- App Wiki presentation splits product graph sections by app ontology, including separate `Code Towers` and `Code Files` sections for `tower` and `code_file` nodes so generated implementation plans stay navigable from the app wiki.
...
- App incremental work items may carry `projectType: "app"` and `appSlice` values such as `product`, `flows`, `screens`, `components`, `data_api`, `capabilities`, `design_system`, `towers_code_files`, and `relationships`. App broad builds must not create `sequence_unit` work items for UX flow planning.
- App readiness diagnostics must use product/UX/implementation language such as screen contracts, API contracts, capability constraints, design system readiness, tower ownership, and code-file planning. Story-shaped diagnostics such as threats, villains, protagonists, lore, kingdoms, hidden truths, or motives must be filtered out for app contexts.
...
- Brand atlas images are generated through the protected `start-world-brand-atlas-image` Edge Function using OpenAI `gpt-image-2`, uploaded to the `project-assets` storage bucket, inserted as `project_assets` image records, and linked back to project wiki metadata through `worldWiki.brandAtlasAssetKey`.
...
- Prompt suggestion generation is project-type aware. App projects must receive product/UX/app-implementation suggestions such as UX flows, screens, components, data/API contracts, capabilities, design systems, towers, and code-file plans; story suggestions such as main threats, villains, protagonists, chapters, lore layers, factions, kingdoms, or inciting events are filtered out for app contexts.
