...
export type SculptRecipe={version:1;levels:SculptLevel[]}|{version:2;levels:SculptLevel[];attachments:SculptAttachment[]}|{version:3;levels:SculptLevel[];attachments:SculptAttachment[]};
...
export function sculptFootprint(shapes:SculptPrimitive[]):SculptPolygon[]{
 ...
 return geometry.map(p=>p.map((r,i)=>{const loop=clean(r as SculptLoop);if((area(loop)>0)!==(i===0))loop.reverse();return loop;}).filter(r=>r.length>=3)).filter(p=>p.length&&Math.abs(area(p[0]))>.5);
}
const anchorSides=['north','south','east','west'] as const;
function wallSource(shapes:SculptPrimitive[],a:[number,number],b:[number,number],nx:number,nz:number):SculptWallAnchor|undefined{
 const x=(a[0]+b[0])/2,z=(a[1]+b[1])/2;
 for(const shape of shapes.filter(s=>s.operation==='add')){
  if(shape.kind==='ellipse'){
   const rx=(x-shape.x)/(shape.width/2),rz=(z-shape.z)/(shape.depth/2);
   if(Math.abs(rx*rx+rz*rz-1)<.13){const angle=(Math.atan2(rz,rx)+Math.PI*2)%(Math.PI*2);return {shapeId:shape.id,side:'curve',u:angle/(Math.PI*2)};}
   continue;
  }
  ...
  for(const side of anchorSides){
   const on=side==='north'?Math.abs(z-front)<.025&&nz>.8&&x>=left-.025&&x<=right+.025:side==='south'?Math.abs(z-back)<.025&&nz<-.8&&x>=left-.025&&x<=right+.025:side==='east'?Math.abs(x-right)<.025&&nx>.8&&z>=back-.025&&z<=front+.025:Math.abs(x-left)<.025&&nx<-.8&&z>=back-.025&&z<=front+.025;
...
function sourcePoint(anchor:SculptWallAnchor,shapes:SculptPrimitive[]):{x:number;z:number}|null{
 const shape=shapes.find(s=>s.id===anchor.shapeId&&s.operation==='add');if(!shape)return null;
...
export function sculptWalls(recipe:SculptRecipe,d:Pick<CityBuildingDesignV3,'floors'|'groundHeight'>):SculptWall[]{
 ...
 for(let floor=0;floor<d.floors;floor++){
  ...
  for(const polygon of sculptFootprint(effectiveSculptShapes(recipe,floor)))for(const [ringIndex,ring] of polygon.entries())for(let i=0;i<ring.length;i++){
   ...
   walls.push({floor,ring:ringIndex,a,b,length,nx:dz/length,nz:-dx/length,angle:Math.atan2(dz,-dx),bottom,top,source:wallSource(shapes,a,b,dz/length,-dx/length)});
  }
  bottom=top;
 }
 return walls;
...
function attachedWall(recipe:SculptRecipe,walls:SculptWall[],a:SculptAttachment){
 if(recipe.version!==3)return nearestWall(walls,a);
...
export function addSculptAttachment(recipe:SculptRecipe,d:Pick<CityBuildingDesignV3,'floors'|'groundHeight'>,attachment:SculptAttachment):{recipe:SculptRecipe;reason:string|null}{
 ...
 if(recipe.version===3&&['door','trim'].includes(attachment.kind)){
 ...
 const next:SculptRecipe=recipe.version===3?{version:3,levels:recipe.levels,attachments:[...old.filter(a=>!single||a.kind!==attachment.kind),attachment]}:{version:2,levels:recipe.levels,attachments:[...old.filter(a=>!single||a.kind!==attachment.kind),attachment]};
...
export function validateSculpt(recipe:SculptRecipe,floors:number):string|null{
 if(![1,2,3].includes(recipe.version)||!Array.isArray(recipe.levels)||recipe.levels.length<1||recipe.levels.length>8||recipe.levels[0].floor!==0)return 'The floor stack is incomplete.';
...
export function resolveSculpt(recipe:SculptRecipe,d:CityBuildingDesignV3):SculptResolved{
 ...
 for(let f=0;f<d.floors;f++){
  const height=f?3:d.groundHeight,top=y+height,polygons=graph?.floors[f].polygons??sculptFootprint(effectiveSculptShapes(recipe,f));floors.push({floor:f,polygons,bottom:y,top});
  const pitched=recipe.version===3&&f===d.floors-1&&d.roof==='pitched'&&sculptPitchedRoofFits(recipe,f);
  if(pitched)pitchedRoof(vertices.roof,effectiveSculptShapes(recipe,f)[0],top-.03);
  for(const [polygonIndex,polygon] of polygons.entries()){
   if(!pitched)surface(vertices.roof,polygon,top-.09,true);surface(vertices.roof,polygon,y+.06,false);
   for(const [ringIndex,ring] of polygon.entries())for(let i=0;i<ring.length;i++){
    ...
    if(len<1.1){wall(0,1,y,top-.16);face(vertices.trim,0,1,top-.16,top);if(f===d.floors-1&&d.roof==='parapet')face(vertices.trim,0,1,top,top+.32);continue;}
 ...
 const kitWalls=d.synarcKit?sculptWalls(recipe,d):[];
 const kit=d.synarcKit&&kitWalls.every(w=>w.source?.side!=='curve')
  ...
  :undefined;
 return {floors,entrance:kit?.entrance??entrance,decorations,kit,vertices,bounds:{minX,maxX,minZ,maxZ}};
}
