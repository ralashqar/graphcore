...
test('narrative RPG mobile seed profile creates playable game-system coverage', () => {
  const profile = getWorldSeedSkeletonProfile('narrative_rpg_mobile')
  const categoryIds = new Set(profile.categories.map((category) => category.id))
  const nodeTypes = new Set(profile.categories.map((category) => category.nodeType))

  assert.equal(profile.projectType, 'game')
  assert.equal(profile.sequence.unitKind, 'quest')
  assert.ok(categoryIds.has('inventory_progression'))
  assert.ok(categoryIds.has('economy_markets'))
  assert.ok(categoryIds.has('travel_links'))
  assert.ok(categoryIds.has('narrative_dialogue'))
  assert.ok(nodeTypes.has('inventory_item'))
  assert.ok(nodeTypes.has('marketplace'))
  assert.ok(nodeTypes.has('travel_link'))
  assert.ok(nodeTypes.has('narrative_scene'))
  assert.match(profile.plannerDirectives.join(' '), /static playable prototype/i)
})
