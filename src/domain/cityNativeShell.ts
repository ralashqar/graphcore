...
export function partitionNativeWall(length:number,height:number,bays:{center:number;width:number}[],doors:{left:number;right:number;bottom:number;top:number}[]):ShellRect[]{
 ...
 const windows:ShellRect[]=bays.filter(b=>!openings.some(o=>b.center+b.width/2>o.left && b.center-b.width/2<o.right)).map(b=>({left:b.center-b.width/2,right:b.center+b.width/2,bottom:0,top:height,kind:"window"}));
