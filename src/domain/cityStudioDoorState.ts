...
/** Leaves of one free door group (`exterior/free/<group>`, `…#2`) open and close together. */
const groupOf=(doorId:string)=>doorId.startsWith('exterior/free/')?doorId.split('#')[0]:null;
...
export function toggleStudioDoor(plotId:string,doorId:string){
 const state=states.get(key(plotId,doorId));if(!state)return false;const target=state.target>.5?0:1,group=groupOf(doorId);state.target=target;
 if(group)for(const [id,other] of states)if(id.startsWith(plotId+'\u0000')&&groupOf(id.slice(plotId.length+1))===group)other.target=target;
 return true;
}
