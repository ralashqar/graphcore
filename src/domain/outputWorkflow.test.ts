...
test('story cinematic requests build scene-graph-assigned parallel animatic graph by default while explicit legacy modes are rejected', () => {
  ...
  assert.match(sequenceAnimaticPlanningPackSource, /scenes_registered/)
...
test('wiki sequence-unit animatics use full chapter screenplay master mode', () => {
  ...
  assert.match(workerSource, /output-workflow-sequence-animatic-director-plan-runtime/)
  ...
  assert.match(sequenceAnimaticPlanningPackSource, /helpers\.safeParseSequenceAnimaticShotPlan\(rawShotPlan\)/)
  assert.match(workerSource, /parseSequenceAnimaticShotPlan: \(value\) => sequenceAnimaticShotPlanSchema\.parse\(value\)/)
  assert.match(sequenceAnimaticPlanningPackSource, /helpers\.parseSequenceAnimaticShotPlan\(\{/)
  ...
  assert.match(sequenceAnimaticPlanningPackSource, /Build final sequence animatic manifest from shot continuity plan|Built final sequence animatic manifest from shot continuity plan/)
...
test('sequence animatic continuity sidecar has typed role, pack schema, and graph contracts', () => {
  ...
  assert.match(workerSource, /output-workflow-sequence-animatic-continuity-batches/)
...
test('sequence animatic shot production graph resolves shared refs before keyframe and video', () => {
  ...
  assert.match(sequenceAnimaticShotReferencePackSource, /orderSequenceAnimaticAssetPackReferences\(helpers\.scopeAssetPackToReferenceAssetKeys/)
...
test('sequence animatic continuity anchors are planned, extracted, and passed to child workflows', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_character_anchor_extract/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_prop_anchor_extract/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_location_anchor_extract/)
  ...
  assert.match(sequenceAnimaticPlanningPackSource, /continuityAnchorIds: anchorIds/)
...
test('cinematic V3 shot parse repairs missing visual refs before validation and manifest assembly', () => {
  ...
  const manifestBlock = sequenceAnimaticPlanningPackSource.slice(sequenceAnimaticPlanningPackSource.indexOf('export async function sequenceAnimaticManifest'), sequenceAnimaticPlanningPackSource.indexOf('export async function sequenceAnimaticManifestArtifact'))
...
test('sequence animatic shot videos use cropped panel keyframe mini graphs', () => {
  ...
  assert.match(sequenceAnimaticShotProductionPackSource, /inferSequenceShotVideoTiming/)
...
test('sequence animatic shot revisions have output-local graph contracts and artifacts', () => {
  ...
  assert.match(sequenceAnimaticShotRevisionPackSource, /planSequenceAnimaticShotRevision/)
