...
test('story cinematic requests build scene-graph-assigned parallel animatic graph by default while explicit legacy modes are rejected', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /scenes_registered/)
...
test('wiki sequence-unit animatics use full chapter screenplay master mode', () => {
  ...
  assert.match(workerSource, /runOpenAiJsonlStream/)
  ...
  assert.match(workerSource, /safeParseSequenceAnimaticShotPlan: \(value\) => \{/)
  assert.match(sequenceAnimaticPackSource, /helpers\.safeParseSequenceAnimaticShotPlan\(rawShotPlan\)/)
  assert.match(workerSource, /parseSequenceAnimaticShotPlan: \(value\) => sequenceAnimaticShotPlanSchema\.parse\(value\)/)
  assert.match(sequenceAnimaticPackSource, /helpers\.parseSequenceAnimaticShotPlan\(\{/)
  ...
  assert.match(sequenceAnimaticPackSource, /Build final sequence animatic manifest from shot continuity plan|Built final sequence animatic manifest from shot continuity plan/)
...
test('sequence animatic continuity sidecar has typed role, pack schema, and graph contracts', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /referenceScope: 'sequence_animatic_shot_keyframe'/)
  assert.match(sequenceAnimaticPackSource, /referenceScope: 'sequence_animatic_coverage_anchor'/)
...
test('sequence animatic shot production graph resolves shared refs before keyframe and video', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shared_asset_ref/)
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_reference_pack/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_shot_reference_pack'\)/)
  assert.match(sequenceAnimaticPackSource, /const scopedReferenceAssetKeys = \[\.\.\.new Set\(\[/)
  assert.match(sequenceAnimaticPackSource, /\.\.\.helpers\.readStringArray\(config\.requiredReferenceAssetKeys\)/)
  assert.match(sequenceAnimaticPackSource, /\.\.\.referenceAssetKeys/)
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_planned_keyframe_prompt/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_planned_keyframe_prompt'\)/)
  assert.match(sequenceAnimaticPackSource, /Reference map/)
  assert.match(sequenceAnimaticPackSource, /Visible subjects/)
  assert.match(sequenceAnimaticPackSource, /Action\/blocking/)
  assert.match(sequenceAnimaticPackSource, /Camera\/framing/)
  assert.match(sequenceAnimaticPackSource, /Lighting\/environment/)
  assert.match(sequenceAnimaticPackSource, /Negative rules/)
  assert.match(workerSource, /Attached image reference order/)
  assert.match(sequenceAnimaticPackSource, /Composition lock: @Image1 is the coverage anchor/)
  assert.match(sequenceAnimaticPackSource, /framing\/background\/blocking source of truth/)
  assert.match(sequenceAnimaticPackSource, /Do not change the coverage-anchor camera angle/)
  assert.match(sequenceAnimaticPackSource, /Create one labeled coverage blockout plate/)
  ...
  assert.match(sequenceAnimaticPackSource, /Create a visual staging brief for one coverage anchor blockout plate/)
  assert.match(sequenceAnimaticPackSource, /Scope: current shot only; ignore unrelated linked setup shots/)
  assert.match(sequenceAnimaticPackSource, /Sparse placement labels and arrows are allowed and required/)
  assert.match(sequenceAnimaticPackSource, /Use named character\/item reference images only to know which placeholders to place/)
  assert.match(sequenceAnimaticPackSource, /No captions, labels, arrows, UI, watermarks, borders, split panels, speech bubbles, or visible text/)
  assert.match(sequenceAnimaticPackSource, /orderSequenceAnimaticAssetPackReferences\(helpers\.scopeAssetPackToReferenceAssetKeys/)
  assert.match(workerSource, /sequenceAnimaticReferenceManifestTextFromRecords\(referenceImageRecords\)/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_video_artifact/)
  ...
  assert.match(sequenceAnimaticPackSource, /Continuity asset image did not produce an asset key/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_continuity_asset_artifact'\)/)
  assert.match(sequenceAnimaticPackSource, /Coverage anchor image did not produce an asset key/)
  assert.match(sequenceAnimaticPackSource, /Shot keyframe image did not produce an asset key/)
  assert.match(sequenceAnimaticPackSource, /function readPreferredUpstreamImage/)
  assert.match(sequenceAnimaticPackSource, /preferredNodeKeys: \['planned_keyframe_image', 'shot_keyframe_image'\]/)
  assert.match(sequenceAnimaticPackSource, /role: 'sequence_animatic_shot_keyframe'/)
...
test('sequence animatic coverage intent batches support fresh scene board coverage grids', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /coverageIntentByShotId/)
  assert.match(sequenceAnimaticPackSource, /sequenceAnimaticZoneCoverageRegistry/)
  assert.match(sequenceAnimaticPackSource, /runStructuredNode/)
  assert.match(sequenceAnimaticPackSource, /output-workflow-sequence-animatic-coverage-runtime/)
...
test('sequence animatic continuity anchors are planned, extracted, and passed to child workflows', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /selectedVisualReferenceKeys/)
  ...
  assert.match(sequenceAnimaticPackSource, /continuityAnchorIds: anchorIds/)
...
test('cinematic V3 shot parse repairs missing visual refs before validation and manifest assembly', () => {
  ...
  const manifestBlock = sequenceAnimaticPackSource.slice(sequenceAnimaticPackSource.indexOf('async function sequenceAnimaticManifest'), sequenceAnimaticPackSource.indexOf('async function sequenceAnimaticManifestArtifact'))
...
test('sequence animatic shot videos use cropped panel keyframe mini graphs', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_input/)
  assert.doesNotMatch(workerSource, /purpose === 'sequence_animatic_shot_input'/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_video_prompt/)
  assert.match(sequenceAnimaticPackSource, /inferSequenceShotVideoTiming/)
  assert.doesNotMatch(workerSource, /purpose === 'sequence_animatic_shot_video_prompt'/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_video/)
  assert.match(sequenceAnimaticPackSource, /executeVideoGeneration/)
  ...
  assert.match(sequenceAnimaticPackSource, /Treat @Image1 as the cropped shot keyframe reference/)
  assert.match(workerSource, /shot video generation requires the cropped shot panel as @Image1/)
  assert.match(sequenceAnimaticPackSource, /includeSpeakerRefs: false/)
  assert.match(sequenceAnimaticPackSource, /offscreenSpeakerVisualReferencesExcluded: true/)
  assert.match(sequenceAnimaticPackSource, /visualReferencePolicy: 'visible_characters_location_props_only'/)
  assert.match(workerSource, /seedanceDirectedControlsSchema/)
  assert.match(workerSource, /buildCompactSeedanceVideoPrompt/)
  assert.match(workerSource, /\[DIRECTED CONTROLS\]/)
  assert.match(sequenceAnimaticPackSource, /directedControls: timing\.directedControls/)
  assert.match(sequenceAnimaticPackSource, /No music, score, audio bed, room tone, crowd wash, or background ambience/)
  assert.match(sequenceAnimaticPackSource, /audioPolicy: 'dialogue_and_direct_diegetic_sfx_only'/)
  ...
  assert.match(sequenceAnimaticPackSource, /ignoredTaggedShotTiming: true/)
...
test('sequence animatic shot revisions have output-local graph contracts and artifacts', () => {
  ...
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_revision_plan/)
  assert.match(sequenceAnimaticPackSource, /planSequenceAnimaticShotRevision/)
  assert.doesNotMatch(workerSource, /purpose === 'sequence_animatic_shot_revision_plan'/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_keyframe_prompt/)
  assert.doesNotMatch(workerSource, /purpose === 'sequence_animatic_shot_keyframe_prompt'/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_keyframe_image/)
  assert.match(sequenceAnimaticPackSource, /executeImageGeneration/)
  assert.match(sequenceAnimaticPackSource, /sequence_animatic_shot_revision_artifact/)
...
test('dynamic cinematic fanout marks obsolete nodes stale instead of deleting active graph nodes', () => {
  ...
  assert.match(source, /async function persistDynamicWorkflowGraphRevision/)
...
test('selected-shot cinematic materialization preserves existing non-target dynamic outputs', () => {
  ...
  assert.match(source, /nodeRows: nodeRows\.map\(preserveNodeRow\)/)
...
test('cinematic v3 dynamic rematerialization preserves durable node outputs and previews', () => {
  ...
  assert.match(source, /persistDynamicWorkflowGraphRevision/)
  assert.match(source, /nodeRows,\s*edgeRows,\s*existingDynamicNodes,\s*dynamicEdgeKeys/s)
})
