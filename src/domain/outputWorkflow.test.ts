...
test('sequence animatic continuity sidecar has typed role, pack schema, and graph contracts', () => {
  ...
  assert.match(sequenceAnimaticContinuityAssetRuntimeSource, /function readTargetVisualBrief/)
  assert.match(sequenceAnimaticContinuityAssetRuntimeSource, /targetNode\.visualDescription/)
  assert.match(sequenceAnimaticContinuityAssetRuntimeSource, /blocks\?\\s\+\[\^\/\]\+\\\/\\s\*shots\?/)
  assert.doesNotMatch(sequenceAnimaticContinuityAssetRuntimeSource, /Shot evidence:\\n/)
  assert.match(sequenceAnimaticContinuityAssetRuntimeSource, /Use the visual brief and project style only/)
  ...
  assert.match(continuityAssetEnsureSource, /rawType === 'crowd'/)
  assert.match(continuityAssetEnsureSource, /rawType === 'group'/)
  assert.match(continuityAssetEnsureSource, /rawType === 'faction'/)
  assert.match(continuityAssetEnsureSource, /reference\.visualDescription/)
...
test('sequence animatic shot production graph uses UI ingredient refs before keyframe only', () => {
  ...
  const focusedShotWorkspaceProjectionSource = readFileSync(resolve(repoRoot, 'src/features/world-builder/animatic/sequenceAnimaticShotWorkspace.ts'), 'utf8')
  const repositorySource = readFileSync(resolve(repoRoot, 'src/data/graphcoreRepository.ts'), 'utf8')
  ...
  assert.match(workerSource, /uiIngredientOverrideMode && shotReferencePackAssetKeys\.length > 0/)
  assert.match(workerSource, /\? shotReferencePackAssetKeys\s*:\s*canonicalKeyframeReferenceAssetKeys\.length > 0/)
  ...
  assert.match(keyframeEnsureSource, /allShotKeyframeJobs/)
  assert.match(keyframeEnsureSource, /keyframePlan\.allShotKeyframeJobs \?\? keyframePlan\.shotKeyframeJobs/)
  assert.match(keyframeEnsureSource, /function keyframePlanContainsRequestedShots/)
  assert.match(keyframeEnsureSource, /!keyframePlanContainsRequestedShots/)
  assert.match(keyframeEnsureSource, /keyframePlanSource = 'combined_scene_plan'/)
  assert.match(shotGraphEnsureSource, /shotReadySource = 'combined_scene_plan'/)
  assert.match(shotGraphEnsureSource, /mergedPlan = mergedShotPlan/)
  assert.match(animaticCommandUtilsSource, /explicitSceneId !== 'sequence_animatic_master'/)
  ...
  assert.equal([...workflowFactorySource.matchAll(/purpose: 'sequence_animatic_planned_keyframe_image',[\s\S]*?quality: 'medium'/g)].length, 2)
  ...
  assert.match(sequenceAnimaticShotReferencePackSource, /sequence_animatic_shot_reference_fix/)
  ...
  assert.match(animaticViewModelSource, /worldReferenceAssetPackEntities/)
  assert.match(animaticViewModelSource, /const assetUrl = assetKeyUrl \|\| existingUrl/)
  assert.match(focusedShotWorkspaceProjectionSource, /assetUrlByAssetKey\.get\(assetKey\)\s*\|\|\s*cleanText/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_shot_reference_pack'\)/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_shot_reference_fix'\)/)
  assert.doesNotMatch(workerSource, /if \(purpose === 'sequence_animatic_shot_reference_fix_apply'\)/)
  ...
  assert.doesNotMatch(keyframeEnsureSource, /Use this attached image exactly/)
  assert.doesNotMatch(shotGraphEnsureSource, /Use this attached image exactly/)
  ...
  assert.match(pageSource, /const sequenceAnimaticRouteActive = Boolean\(activeWikiEntityPage\?\.animaticRequestId\)/)
  assert.match(pageSource, /if \(sequenceAnimaticRouteActive\) return \[\]/)
  assert.match(pageSource, /onBindShotElement=\{\(\) => \{\}\}/)
  ...
  assert.doesNotMatch(focusedWorkspaceSource, /onBindShotElement\(shotElementKey/)
  assert.doesNotMatch(focusedWorkspaceSource, /activeButtonRef\.current\?\.scrollIntoView/)
  assert.match(focusedWorkspaceSource, /world-wiki-sequence-shot-inline-prompt/)
  assert.match(focusedWorkspaceSource, /WorkflowActiveNodeStrip/)
  assert.match(focusedWorkspaceSource, /WorkflowProgressSummary/)
  ...
  assert.match(focusedWorkspaceSource, /shotKeyframeInFlightForTimelineItem[\s\S]*busyRunKeys\.has\(itemRunKey\) \|\| item\.shot\.keyframeRunning[\s\S]*return false/)
  assert.doesNotMatch(focusedWorkspaceSource, /shotKeyframeInFlightForTimelineItem[\s\S]*?keyframeDependencyRunning[\s\S]*?return Boolean/)
  assert.match(focusedWorkspaceSource, /world-wiki-shot-panel-focus__pending/)
  assert.doesNotMatch(focusedWorkspaceSource, /world-wiki-shot-panel-focus__generating-overlay/)
  assert.match(focusedWorkspaceSource, /is-keyframe-generating/)
  assert.match(routeViewerSource, /workflowProgressForRequest={workflowProgressForRequest}/)
  ...
  assert.match(wikiShellStylesSource, /world-wiki-shot-context-grid[\s\S]*overflow-anchor: none/)
  assert.match(wikiShellStylesSource, /world-wiki-shot-context-grid \.ghost-button:hover[\s\S]*transform: none/)
...
test('sequence animatic state polling avoids full workflow run-step payloads', () => {
  ...
  assert.match(repositorySource, /collectSequenceAnimaticStateAssetKeys\(input: \{\s*requests\?: OutputRequest\[\]/)
  assert.match(repositorySource, /for \(const request of input\.requests \?\? \[\]\) \{\s*collectOutputAssetReferences\(request\.metadata, known, keys\)/)
  assert.match(repositorySource, /collectSequenceAnimaticStateAssetKeys\(\{ requests, artifacts, runs: runsBeforeAssetHydration, projections \}\)/)
...
test('sequence animatic keyframe ensure supports shot-scoped next actions', () => {
  ...
  const shotGraphParsed = sequenceAnimaticShotProductionGraphEnsureResponseSchema.parse({
    ...
    coverageSetupId: 'setup_a',
    dependencyNodeIds: ['set_a', 'zone_a'],
    graphPolicyVersion: 'primary_chain_v14_reference_fix',
  })
...
test('sequence animatic shot videos use focused ingredient reference graphs', () => {
  const workerSource = readFileSync(resolve(repoRoot, 'supabase/functions/_shared/output-workflow.ts'), 'utf8')
  const mediaRuntimeSource = readFileSync(resolve(repoRoot, 'supabase/functions/_shared/output-workflow-media-runtime.ts'), 'utf8')
  const sequenceAnimaticShotReferencePackSource = readFileSync(resolve(repoRoot, 'supabase/functions/_shared/output-workflow-sequence-animatic-shot-reference-pack.ts'), 'utf8')
  const sequenceAnimaticShotProductionPackSource = readFileSync(resolve(repoRoot, 'supabase/functions/_shared/output-workflow-sequence-animatic-shot-production-pack.ts'), 'utf8')
  ...
  assert.match(factorySource, /sequence_animatic_shot_video_graph_v3_named_director_prompt/)
  assert.match(factorySource, /shot_input[\s\S]*shot_video_reference_pack[\s\S]*shot_video_prompt_plan[\s\S]*shot_video_prompt[\s\S]*shot_video[\s\S]*shot_video_artifact/)
  assert.match(factorySource, /shotVideoReferenceOverride/)
  assert.match(factorySource, /ui_ingredient_ref_/)
  ...
  assert.match(sequenceAnimaticShotProductionPackSource, /sequence_animatic_shot_video_prompt_plan/)
  assert.match(sequenceAnimaticShotProductionPackSource, /subjectRoster/)
  assert.match(sequenceAnimaticShotProductionPackSource, /timeBeats/)
  assert.match(sequenceAnimaticShotProductionPackSource, /directorShot/)
  assert.match(sequenceAnimaticShotProductionPackSource, /buildSequenceAnimaticShotVideoDirectorPrompt/)
  assert.match(sequenceAnimaticShotProductionPackSource, /\[SHOT DIRECTION\]/)
  assert.match(sequenceAnimaticShotProductionPackSource, /\[AUDIO \/ CONSTRAINTS\]/)
  ...
  assert.doesNotMatch(sequenceAnimaticShotProductionPackSource, /00:00-end/)
  assert.match(sequenceAnimaticShotProductionPackSource, /dialogueDeliveryCues/)
  assert.match(sequenceAnimaticShotProductionPackSource, /characterPerformanceCues/)
  assert.match(sequenceAnimaticShotProductionPackSource, /performanceDirection/)
  assert.match(sequenceAnimaticShotProductionPackSource, /dialogueDeliveryPlan/)
  assert.match(sequenceAnimaticShotProductionPackSource, /Performance \/ Acting/)
  assert.match(sequenceAnimaticShotProductionPackSource, /delivery \${cue\.delivery}/)
  assert.match(sequenceAnimaticShotProductionPackSource, /subtext \${cue\.subtext}/)
  assert.match(sequenceAnimaticShotProductionPackSource, /voice \${cue\.voiceEnergy}/)
  assert.match(sequenceAnimaticShotProductionPackSource, /isGenericVideoReferenceName/)
  assert.match(sequenceAnimaticShotProductionPackSource, /displayName/)
  assert.match(sequenceAnimaticShotProductionPackSource, /Rejected generic video prompt-plan reference name/)
  assert.match(sequenceAnimaticShotReferencePackSource, /preferredReferenceDisplayName/)
  assert.match(sequenceAnimaticShotReferencePackSource, /sequence_animatic_shot_video_graph_v3_named_director_prompt/)
  assert.doesNotMatch(sequenceAnimaticShotProductionPackSource, /const referenceBindingLines/)
  ...
  assert.match(sequenceAnimaticShotProductionPackSource, /No shot keyframe is attached; use the ingredient references/)
  ...
  assert.match(shotVideoRuntimeSource, /performanceAxisLabel/)
  assert.match(shotVideoRuntimeSource, /delivery \${readText\(line\.delivery\)}/)
  assert.match(shotVideoRuntimeSource, /subtext \${readText\(line\.subtext\)}/)
  assert.match(shotVideoRuntimeSource, /voiceEnergy/)
  ...
  assert.match(shotVideoHookSource, /buildSequenceAnimaticShotVideoReferenceOverride/)
