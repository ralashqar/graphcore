...
function SequenceAnimaticContinuityStructureModal({
  ...
  const nodeCountLabel = `${sets.length} set${sets.length === 1 ? '' : 's'} / ${zones.length} zone${zones.length === 1 ? '' : 's'} / ${spots.length} spot${spots.length === 1 ? '' : 's'} / ${angles.length} angle${angles.length === 1 ? '' : 's'}`
      ...
      <div className="world-wiki-sequence-continuity-structure-body">
        {!hasStructure && !hasTemporaryRefs ? (
          <div className="world-wiki-sequence-continuity-empty">
            ...
            <p>Generate continuity structure first, then this view will show sets, zones, spots, angles, props, and temporary characters.</p>
          </div>
        ) : null}
        {hasStructure ? (
          <section className="world-wiki-sequence-continuity-section">
            <div className="world-wiki-sequence-continuity-section-head">
              ...
              <span>Sets, zones, spots, and camera angles</span>
            </div>
...
function buildSequenceAnimaticContinuityAnchorViews(input: {
  ...
  return [...mergedByKey.values()].filter((anchor) => {
    const anchorType = trimOptionalString(anchor.anchorType) || trimOptionalString(anchor.type)
    return anchorType === 'character' || anchorType === 'prop'
  }).map((anchor): SequenceAnimaticContinuityAnchorView => {
    const id = trimOptionalString(anchor.id)
    const anchorType = trimOptionalString(anchor.anchorType) || trimOptionalString(anchor.type)
    const type: SequenceAnimaticContinuityAnchorView['type'] = anchorType === 'character' ? 'character' : 'prop'
    ...
    const view: Omit<SequenceAnimaticContinuityAnchorView, 'usageLabel' | 'usageDetailLabel'> = {
      ...
      type,
      typeLabel: type === 'character' ? 'Temporary character' : 'Prop',
      iconId: iconForWorldEntity(type === 'character' ? 'actor' : 'object'),
      thumbnailUrl,
      status,
      statusLabel,
      progressLabel,
      sourceAtlasNodeLabel: type === 'character' ? 'Character Anchor Atlas' : type === 'prop' ? 'Prop Anchor Atlas' : 'Location Anchor Atlas',
      shotIds: readLooseArray(anchor.shotIds).map(trimOptionalString).filter(Boolean),
      blockIds: readLooseArray(anchor.storyboardBlockIds).map(trimOptionalString).filter(Boolean),
    }
    return {
      ...view,
...
function sequenceAnimaticShotBindingLabels(binding: Record<string, unknown>, shot: Record<string, unknown>) {
  const setId = trimOptionalString(binding.setId) || trimOptionalString(shot.continuitySetId)
  const zoneId = trimOptionalString(binding.zoneId) || trimOptionalString(shot.continuityZoneId)
  const angleId = trimOptionalString(binding.angleId) || trimOptionalString(shot.continuityAngleId)
  ...
  const spotIds = readLooseArray(binding.spotIds)
    .map(trimOptionalString)
    .filter((spotId) => spotId && !sequenceAnimaticSpatialBindingLooksShotDerived(spotId, shot) && !suspectZone)
  const worldLocationRefId = trimOptionalString(binding.worldLocationRefId) || trimOptionalString(shot.worldLocationRefId) || trimOptionalString(shot.locationRefId)
  if (suspectZone || suspectAngle) {
    ...
    return {
      ...
      detail: detail || 'Continuity zone or angle looked like a shot title/action instead of a physical set.',
    }
  }
  const label = [
    zoneId ? displayNameFromRefId(zoneId) : '',
    angleId ? displayNameFromRefId(angleId) : '',
  ].filter(Boolean).join(' / ')
  const detail = [
    worldLocationRefId ? `Location ${displayNameFromRefId(worldLocationRefId)}` : '',
    setId ? `Set ${displayNameFromRefId(setId)}` : '',
...
function buildSequenceAnimaticViewModel(input: {
  ...
  continuityArtifacts.forEach((artifact) => {
    const metadata = readLooseRecord(artifact.metadata)
  ...
  const continuityLocationSets = readLooseArray(continuityLocationSource.locationSets ?? continuityLocationSource.location_sets).map(readLooseRecord).map((entry): SequenceAnimaticContinuityLocationView => ({
    ...
    .concat(readLooseArray(continuityGraphV2.zones).map(readLooseRecord).map((entry): SequenceAnimaticContinuityLocationView => ({
      id: trimOptionalString(entry.id),
      name: trimOptionalString(entry.name) || displayNameFromRefId(trimOptionalString(entry.id)),
    ...
    .concat(readLooseArray(continuityGraphV2.spots).map(readLooseRecord).map((entry): SequenceAnimaticContinuityLocationView => ({
      id: trimOptionalString(entry.id),
      name: trimOptionalString(entry.name) || displayNameFromRefId(trimOptionalString(entry.id)),
      summary: trimOptionalString(entry.visualBrief),
      kind: 'spot',
  ...
  const continuityAngleSourceEntries = readLooseArray(continuityGraphV2.angles).length > 0
  ...
  const continuityAssetTargets = (() => {
    const entries = [
      ...continuityLocationSets.map((entry) => ({
        nodeId: entry.id,
        name: entry.name,
        assetKind: entry.kind === 'set' ? 'location_set' : entry.kind === 'zone' ? 'location_zone' : entry.kind === 'angle' ? 'location_angle' : 'location_spot',
...
export function WorldGraphPage({
  ...
  onDeriveSequenceAnimaticContinuityBlock,
  onDeriveSequenceAnimaticContinuityStructure,
  ...
  const handleDeriveSequenceAnimaticBlockContinuity = useCallback(async (
    ...
    const runKey = `${model.request.id}:${block.id}:derive_continuity`
    if (sequenceAnimaticBlockRunKey) return
    setSequenceAnimaticBlockRunKey(runKey)
    try {
      await onDeriveSequenceAnimaticContinuityBlock({
  ...
  const handleDeriveSequenceAnimaticContinuityStructure = useCallback(async (model: SequenceAnimaticViewModel) => {
    ...
    setSequenceAnimaticBlockRunKey(runKey)
    try {
      const shouldGenerate = model.continuityGraphStatus === 'empty' || model.continuityCoverageLabel === 'No shot bindings'
      const shouldFillGaps = /\bneed binding\b/i.test(model.continuityStructureStatusLabel)
      await onDeriveSequenceAnimaticContinuityStructure({
        masterRequestId: model.request.id,
        continuityRequestId: model.continuityRequest?.id ?? null,
        mode: model.continuityStale
          ? 'regenerate'
          : shouldGenerate
            ? 'generate'
            : shouldFillGaps
  ...
  const handleRunSequenceAnimaticContinuityAssets = useCallback(async (
    ...
    setSequenceAnimaticBlockRunKey(runKey)
    try {
      if (!model.continuityRequest?.id) {
        throw new Error('Prepare and derive continuity structure before generating continuity assets.')
      }
      ...
      for (const target of targets) {
        const ensureResult = await onEnsureSequenceAnimaticContinuityAssetWorkflow({
          masterRequestId: model.request.id,
          continuityRequestId: model.continuityRequest.id,
          nodeId: target.nodeId,
          mode: target.status === 'stale' || target.status === 'ready' ? 'regenerate' : 'generate',
        })
        ...
        await onStartOutputWorkflowRun({
          ...
          prompt: assetRequest.prompt || `Generate continuity asset for ${target.name}.`,
          targetFormat: 'image',
          selectedSequenceUnitKeys: model.request.selectedSequenceUnitKeys,
          input: {
            ...readLooseRecord(existingRun?.input),
          ...
          metadata: {
            ...
            runMode: 'sequence_animatic_continuity_asset',
            runScope: 'upstream_to_node',
            targetNodeKeys: ['continuity_asset_artifact'],
            forceNodeKeys: ['continuity_asset_prompt', 'continuity_asset_image', 'continuity_asset_artifact'],
            reuseExistingUpstreamOutputs: true,
            allowStaleUpstreamOutputs: true,
            debugSkipVideoGeneration: false,
            cinematicVideoApproved: false,
            sourceRunId: existingRun?.id ?? assetRequest.latestRunId ?? model.continuityRequest.latestRunId,
            parentRequestId: model.continuityRequest.id,
            masterRequestId: model.request.id,
              ...
              {routeAnimaticModel ? (
                <div className="world-wiki-sequence-animatic-timeline">
                  <section className="world-wiki-sequence-animatic-continuity-strip" aria-label="Continuity refs">
                    ...
                    <span>{routeAnimaticModel.continuityStatusLabel} / {routeAnimaticModel.continuityCoverageLabel}</span>
                    <span>{routeAnimaticModel.continuityStructureStatusLabel}</span>
                    ...
                    <div className="world-wiki-sequence-animatic-continuity-strip-actions">
                      <button
                        ...
                        disabled={routeAnimaticModel.continuityStructureRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:derive_continuity_structure`}
                        onClick={() => void handleDeriveSequenceAnimaticContinuityStructure(routeAnimaticModel)}
                        type="button"
                      >
                        {routeAnimaticModel.continuityStructureRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:derive_continuity_structure`
                          ? <><span className="world-mini-spinner" aria-hidden="true" />Generating structure</>
                          : routeAnimaticModel.continuityStructureActionLabel}
                      </button>
                      <button
                        className="ghost-button compact"
                        disabled={routeAnimaticModel.continuityRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_pack` || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_assets`}
                        onClick={() => {
                          if (routeAnimaticModel.continuityReady && routeAnimaticModel.continuityAssetTargets.length > 0) {
                            void handleRunSequenceAnimaticContinuityAssets(routeAnimaticModel)
                            return
                          }
                          void handleRunSequenceAnimaticContinuity(routeAnimaticModel)
                        }}
                        type="button"
                      >
                        {routeAnimaticModel.continuityRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_pack` || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_assets`
                          ? <><span className="world-mini-spinner" aria-hidden="true" />Generating</>
                          : routeAnimaticModel.continuityButtonLabel}
                      </button>
                      <button
                        className="ghost-button compact"
                        onClick={() => setSequenceAnimaticContinuityInspector(routeAnimaticModel)}
                        type="button"
                      >
                    ...
                    {routeAnimaticModel.continuityAssetTargets.length > 0 ? (
                      <div className="world-wiki-sequence-animatic-continuity-assets">
                        {routeAnimaticModel.continuityAssetTargets.slice(0, 12).map((target) => (
                          <span key={target.nodeId} className={`is-${target.status}`} title={`${target.name}: ${target.statusLabel}`}>
                            {target.assetUrl ? <img src={target.assetUrl} alt="" /> : <EntityIcon id={target.assetKind.includes('location') ? 'environment' : target.assetKind === 'prop' ? 'item' : 'character'} />}
                            <em>{target.name}</em>
                    ...
                    {routeAnimaticModel.continuityLocationSets.length + routeAnimaticModel.continuityLocationAngles.length > 0 ? (
                      <div className="world-wiki-sequence-animatic-continuity-strip-graph">
                        {[...routeAnimaticModel.continuityLocationSets, ...routeAnimaticModel.continuityLocationAngles].slice(0, 8).map((entry) => (
                          <span key={entry.id || entry.name} title={entry.summary}>
                            {entry.kind ? `${entry.kind}: ` : ''}{entry.name}
                          </span>
                        ))}
                      </div>
                    ) : null}
                    ...
                    >
                      <header className="world-wiki-sequence-animatic-block-toolbar">
                        <div>
                          ...
                          <small className={`world-wiki-sequence-animatic-block-continuity-state is-${block.continuityBlockStatus}`}>
                            {block.continuityBlockStatus === 'deriving' ? <span className="world-mini-spinner" aria-hidden="true" /> : null}
                            {block.continuityBlockStatusLabel}
                            {block.continuityBlockError ? ` / ${block.continuityBlockError}` : ''}
                          </small>
                          <small className="world-wiki-sequence-animatic-block-asset-state">
                            {block.continuityAssetCountLabel}
                          </small>
                        </div>
                        <div>
                          <button
                            className="ghost-button compact"
                            disabled={block.isProvisional || block.continuityBlockStatus === 'deriving' || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:${block.id}:derive_continuity`}
                            onClick={() => void handleDeriveSequenceAnimaticBlockContinuity(routeAnimaticModel, block)}
                            type="button"
                          >
                            {block.continuityBlockStatus === 'deriving' || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:${block.id}:derive_continuity`
                              ? <><span className="world-mini-spinner" aria-hidden="true" />Deriving</>
                              : block.continuityBlockActionLabel}
                          </button>
                          <button className="ghost-button compact world-wiki-sequence-animatic-debug-action" disabled={block.isProvisional || sequenceAnimaticGraphOpenKey === `${routeAnimaticModel.request.id}:${block.id}:block_graph`} onClick={() => void handleOpenSequenceAnimaticBlockGraph(routeAnimaticModel, block)} type="button">
                            {sequenceAnimaticGraphOpenKey === `${routeAnimaticModel.request.id}:${block.id}:block_graph` ? <><span className="world-mini-spinner" aria-hidden="true" />Opening</> : 'Graph'}
                          </button>
                          {block.continuityAssetTargets.length > 0 ? (
                            <button
                              className="ghost-button compact"
                              disabled={sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_assets` || block.continuityAssetTargets.every((target) => target.status === 'ready')}
                              onClick={() => void handleRunSequenceAnimaticContinuityAssets(routeAnimaticModel, block.continuityAssetTargets)}
                              type="button"
                            >
                              {sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:continuity_assets`
                                ? <><span className="world-mini-spinner" aria-hidden="true" />Assets</>
                                : block.continuityAssetTargets.some((target) => target.status === 'stale') ? 'Regenerate stale assets' : 'Generate block assets'}
                            </button>
                          ) : null}
                          <button className="ghost-button compact" disabled={block.isProvisional || block.storyboardRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:${block.id}:regenerate_storyboard`} onClick={() => void handleRunSequenceAnimaticBlock(routeAnimaticModel, block, 'regenerate_storyboard')} type="button">
                          ...
                          {block.videoReady && block.videoUrl ? (
                            ...
                            <button className="ghost-button compact" disabled={block.isProvisional || block.videoRunning || sequenceAnimaticBlockRunKey === `${routeAnimaticModel.request.id}:${block.id}:generate_video`} onClick={() => void handleRunSequenceAnimaticBlock(routeAnimaticModel, block, 'generate_video')} type="button">
                      ...
                      {block.continuityAssetTargets.length > 0 ? (
                        <div className="world-wiki-sequence-animatic-block-assets">
                          {block.continuityAssetTargets.slice(0, 10).map((target) => (
                            <span key={target.nodeId} className={`is-${target.status}`} title={`${target.assetKind.replace(/_/g, ' ')}: ${target.name}`}>
                              {target.assetUrl ? <img src={target.assetUrl} alt="" /> : <EntityIcon id={target.assetKind.includes('location') ? 'environment' : target.assetKind === 'prop' ? 'item' : 'character'} />}
                              ...
                              <div className="world-wiki-sequence-animatic-shot-text">
                                ...
                                <div className="world-wiki-sequence-shot-detail-rows">
                                  <div className="world-wiki-sequence-shot-core-details">
                                    ...
                                    <button className="world-wiki-sequence-shot-detail-row is-core-detail" title={shot.spatialContinuityDetail || shot.spatialContinuityLabel} onClick={() => setSequenceAnimaticShotInspector({ kind: 'camera', blockTitle: block.title, shotTitle: `${shot.title} / spatial continuity`, content: [shot.spatialContinuityLabel, shot.spatialContinuityDetail].filter(Boolean).join('\n') || 'No spatial continuity binding recorded.' })} type="button">
                                      <EntityIcon id="environment" />
                                      <strong>Set</strong>
      ...
      {sequenceAnimaticPreviewModel && !activeWikiEntityPage?.animaticRequestId ? (
        <div className="world-wiki-sequence-animatic-overlay" onClick={() => setSequenceAnimaticPreviewRequestId(null)}>
          ...
          >
            ...
            <section className="world-wiki-sequence-animatic-continuity-panel" aria-label="Continuity refs">
              <div className="world-wiki-sequence-animatic-continuity-head">
                <div>
                  ...
                  <small>{sequenceAnimaticPreviewModel.continuityStatusLabel} / {sequenceAnimaticPreviewModel.continuityCoverageLabel} / {sequenceAnimaticPreviewModel.continuityStructureStatusLabel}</small>
                </div>
                <button
                  className="primary-button compact"
                  disabled={sequenceAnimaticPreviewModel.continuityStructureRunning || sequenceAnimaticBlockRunKey === `${sequenceAnimaticPreviewModel.request.id}:derive_continuity_structure`}
                  onClick={() => void handleDeriveSequenceAnimaticContinuityStructure(sequenceAnimaticPreviewModel)}
                  type="button"
                >
                  {sequenceAnimaticPreviewModel.continuityStructureRunning || sequenceAnimaticBlockRunKey === `${sequenceAnimaticPreviewModel.request.id}:derive_continuity_structure`
                    ? <><span className="world-mini-spinner" aria-hidden="true" />Generating structure</>
                    : sequenceAnimaticPreviewModel.continuityStructureActionLabel}
                </button>
                <button
                  className="ghost-button compact"
                ...
                <button
                  className="ghost-button compact"
                  onClick={() => setSequenceAnimaticContinuityInspector(sequenceAnimaticPreviewModel)}
                  type="button"
                >
                  <EntityIcon id="environment" />
                  Structure
              ...
              {([
                ...
                <div key={label} className="world-wiki-sequence-animatic-continuity-group">
                  ...
                  <div className="world-wiki-sequence-animatic-continuity-list">
                    {anchors.map((anchor) => (
                      <article key={anchor.id} className={`world-wiki-sequence-animatic-continuity-card is-${anchor.status}`}>
                        <div className="world-wiki-sequence-animatic-continuity-thumb">
                          {anchor.thumbnailUrl ? <img src={anchor.thumbnailUrl} alt="" /> : <EntityIcon id={anchor.iconId} />}
                        </div>
                        <div>
                          <strong>{anchor.name}</strong>
                          <em>{anchor.typeLabel} / {anchor.usageLabel}</em>
                          <small>{anchor.usageDetailLabel}</small>
                          <small>{anchor.progressLabel}</small>
                        </div>
                        <span className="world-wiki-sequence-animatic-continuity-status">{anchor.statusLabel}</span>
                      </article>
                    ))}
                  </div>
                </div>
              ...
              <div className="world-wiki-sequence-animatic-document">
                  ...
                  <section key={block.id} className="world-wiki-sequence-animatic-block">
                    <div className="world-wiki-sequence-animatic-block-head">
                      ...
                      <div>
                        <button
                          className="ghost-button compact"
                          disabled={block.isProvisional || sequenceAnimaticGraphOpenKey === `${sequenceAnimaticPreviewModel.request.id}:${block.id}:block_graph`}
                        ...
                        <button
                          className="ghost-button compact"
                          disabled={block.isProvisional || block.storyboardRunning || sequenceAnimaticBlockRunKey === `${sequenceAnimaticPreviewModel.request.id}:${block.id}:regenerate_storyboard`}
                        ...
                        {block.videoReady && block.videoUrl ? (
                          ...
                          <button
                            className="ghost-button compact world-wiki-sequence-animatic-video-action"
                            disabled={block.isProvisional || block.videoRunning || sequenceAnimaticBlockRunKey === `${sequenceAnimaticPreviewModel.request.id}:${block.id}:generate_video`}
                ...
                {sequenceAnimaticPreviewModel.continuityLocationSets.length + sequenceAnimaticPreviewModel.continuityLocationAngles.length + sequenceAnimaticPreviewModel.continuityAnchors.characters.length + sequenceAnimaticPreviewModel.continuityAnchors.props.length + sequenceAnimaticPreviewModel.continuityAnchors.locationSpots.length > 0 ? (
                  <section className="world-wiki-sequence-animatic-new-refs">
                    ...
                    <div className="world-wiki-sequence-animatic-new-ref-grid">
                      {[...sequenceAnimaticPreviewModel.continuityLocationSets, ...sequenceAnimaticPreviewModel.continuityLocationAngles].slice(0, 12).map((entry) => (
                        <article key={`${entry.kind}:${entry.id || entry.name}`}>
                          <EntityIcon id="environment" />
                          <strong>{entry.name}</strong>
                          <em>{entry.kind ?? 'scene'} / {entry.shotIds.length > 0 ? `${entry.shotIds.length} shot${entry.shotIds.length === 1 ? '' : 's'}` : 'no shot usage'}</em>
                        </article>
                      ))}
                      {[...sequenceAnimaticPreviewModel.continuityAnchors.characters, ...sequenceAnimaticPreviewModel.continuityAnchors.props, ...sequenceAnimaticPreviewModel.continuityAnchors.locationSpots].slice(0, 12).map((anchor) => (
                        <article key={`${anchor.type}:${anchor.id}`}>
                          {anchor.thumbnailUrl ? <img src={anchor.thumbnailUrl} alt="" /> : <EntityIcon id={anchor.iconId} />}
                          <strong>{anchor.name}</strong>
                          <em>{anchor.typeLabel} / {anchor.usageLabel}</em>
                        </article>
