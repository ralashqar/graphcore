import {CityStudio} from './CityStudio';
import {studioEnabled} from '../../domain/cityStudioTypes';
import {useEffect,useMemo,useRef,useState} from 'react';
import {Html,OrbitControls} from '@react-three/drei';
import {useFrame,useThree,type ThreeEvent} from '@react-three/fiber';
import {Group,MOUSE,Plane,Raycaster,Vector2,Vector3,type PerspectiveCamera} from 'three';
...
function LegacyCityLandEditor({land,session,camera,reduced}:{land:CityLandController;session:ExplorationSession;camera:PerspectiveCamera;reduced:boolean}){
 ...
 useEffect(()=>{if(!construction)return;const preventMenu=(event:MouseEvent)=>event.preventDefault();gl.domElement.addEventListener('contextmenu',preventMenu);return()=>gl.domElement.removeEventListener('contextmenu',preventMenu);},[construction,gl]);
 ...
 return <>
 {construction&&<OrbitControls ref={controls} camera={camera} target={target} mouseButtons={{LEFT:undefined,MIDDLE:MOUSE.PAN,RIGHT:MOUSE.ROTATE}} enablePan enabled={!dragging} minDistance={plot.size*.55} maxDistance={plot.size*4} minPolarAngle={.12} maxPolarAngle={1.35} enableDamping dampingFactor={.12}/>}
 ...
 {showSculpt&&<CityVolumeSculptControls land={land} plot={plot} draft={draft} selected={land.selectedVolume} setSelected={land.setSelectedVolume} orbitEnabled={enabled=>{if(controls.current)controls.current.enabled=enabled;}} viewTopDown={()=>{camera.position.set(center.x,plot.size*2.1,center.z+.01);controls.current?.target.set(center.x,.1,center.z);controls.current?.update();invalidate();}}/>}
 {showTiles&&<CityLandTiles land={land} plot={plot} draft={draft} selectedVolume={land.selectedVolume}/>}
