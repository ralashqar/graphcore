...
import './cityLand.css';
import {CitySculptControls,startSculpt} from './CitySculptControls';
...
export function CityLandEditor({land,session,camera,reduced}:{land:CityLandController;session:ExplorationSession;camera:PerspectiveCamera;reduced:boolean}){
 const {gl,invalidate}=useThree(),controls=useRef<OrbitControlsImpl>(null),[mode,setMode]=useState<typeof MODES[number]>('Building'),[category,setCategory]=useState('All'),[dragging,setDragging]=useState(false),[ghost,setGhost]=useState<CityBuildingDesignV3|null>(null);
 ...
 {construction&&!sculpting&&!land.previewStatus.error&&mode==='Building'&&<group rotation={[0,plot.rotation*Math.PI/2,0]} position={[center.x,0,center.z]}>
 ...
 {construction&&sculpting&&mode==='Building'&&<CitySculptControls land={land} plot={plot} draft={draft} orbitEnabled={enabled=>{if(controls.current)controls.current.enabled=enabled;}} viewTopDown={()=>{camera.position.set(center.x,plot.size*2.1,center.z+.01);controls.current?.target.set(center.x,.1,center.z);controls.current?.update();invalidate();}}/>}
