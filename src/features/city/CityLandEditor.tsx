...
export function CityLandEditor({land,session,camera,reduced}:{land:CityLandController;session:ExplorationSession;camera:PerspectiveCamera;reduced:boolean}){
 ...
 {construction&&!land.previewStatus.error&&mode==='Building'&&<group rotation={[0,plot.rotation*Math.PI/2,0]} position={[center.x,0,center.z]}>
 ...
 <footer className="city-land-bottom"><nav aria-label="Construction modes">{MODES.map(m=><button key={m} aria-pressed={mode===m} onClick={()=>setMode(m)}>{m}</button>)}</nav>{mode==='Building'&&<><label className="land-category">Building type <select value={category} onChange={e=>setCategory(e.target.value)}>{['All',...new Set(COMPOSITIONS.map(p=>presetCategory(p.patch.archetype)))].map(c=><option key={c}>{c}</option>)}</select></label><div className="city-land-carousel">{COMPOSITIONS.map((p,i)=>category!=='All'&&presetCategory(p.patch.archetype)!==category?null:<button key={p.name} onClick={()=>{const design=applyComposition(d,i);land.editPreset({...draft,design:{...design,rotation:plot.rotation,enclosure:d.enclosure}});}}><img src={`/city/presets/${p.name.toLowerCase().replaceAll(' ','-')}.webp`} alt="" loading="lazy" onError={e=>{e.currentTarget.style.display='none';}}/><span>{p.name}</span></button>)}</div></>}</footer>
 </>}
