...
import {curveRayHit,curveXAt} from '../../domain/cityStudioFaceCurve';
import {bendPoint,bendPose} from '../../domain/cityStudioCurvedWalls';

export const PAINT_BRUSHES=[{size:.5,label:'Small'},{size:1,label:'Medium'},{size:2,label:'Large'}] as const;
/** Curved faces also carry `curve`/`bend` (x is arc length); their `rotation` is the wall's facing at `x`. */
export type OwnedPaintHit={faceId:string;shapeId:string;side:string;x:number;y:number;length:number;height:number;origin:[number,number];rotation:number;base:number}&Partial<Pick<StudioFreeFace,'curve'|'bend'>>;
type Frame=Pick<StudioFreeFace,'id'|'shapeId'|'side'|'origin'|'rotation'|'base'|'length'|'height'>&Partial<Pick<StudioFreeFace,'curve'|'bend'>>;

const toHit=(f:Frame,x:number,y:number):OwnedPaintHit=>({faceId:f.id,shapeId:f.shapeId,side:f.side,x,y,length:f.length,height:f.height,origin:f.origin,rotation:f.bend?bendPose(f.bend,x).rotation:f.rotation,base:f.base,...(f.curve&&f.bend?{curve:f.curve,bend:f.bend}:{})});
...
export function ownedPaintHit(plotId:string,anchor:{shapeId:string;side:string},point:{x:number;y:number;z:number}):OwnedPaintHit|null{
 ...
 if(f.curve)return toHit(f,curveXAt(f.curve,point.x,point.z),point.y-f.base);
...
export function faceRayHit(f:OwnedPaintHit,ray:Ray):OwnedPaintHit|null{
 if(f.curve){const p=curveRayHit(f.curve,FREE_FACE.thickness/2,ray.origin,ray.direction);if(!p)return null;const x=curveXAt(f.curve,p.x,p.z);return {...f,x,y:p.y-f.base,rotation:f.bend?bendPose(f.bend,x).rotation:f.rotation};}
...
export function facePoint(f:Pick<OwnedPaintHit,'origin'|'rotation'|'base'|'bend'>,x:number,y:number,out=FREE_FACE.thickness/2+.06):[number,number,number]{
 if(f.bend){const p=bendPoint(f.bend,x,0,out);return [p[0],f.base+y,p[2]];}
