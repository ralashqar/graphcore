...
export type LandPhase='exploring'|'staging'|'inspection'|'purchasing'|'celebration'|'construction';
export function useCityLand(enabled:boolean,properties:CityProperty[],capacity:number,size:24|48){
 ...
 const [world,setWorld]=useState<LandWorld|null>(null),[phase,setPhase]=useState<LandPhase>('exploring'),[selected,setSelected]=useState<LandPlot|null>(null),[draft,setDraft]=useState<LandDraft|null>(null),[error,setError]=useState(''),[saving,setSaving]=useState(false),[dirty,setDirty]=useState(false),[history,setHistory]=useState<LandDraft[]>([]),[future,setFuture]=useState<LandDraft[]>([]);
 ...
 useEffect(()=>{if(!dirty||phase!=='construction')return;const t=setTimeout(()=>void persist(),600);return()=>clearTimeout(t);},[draft,dirty,phase]);
 ...
 const edit=(next:LandDraft,record=true)=>{const p=selectedRef.current;if(!p)return;next={...next,design:fitLandDesign(next.design,p.rotation)};const previous=draftRef.current;if(record&&previous){setHistory(h=>[...h.slice(-49),previous]);setFuture([]);}editVersion.current++;draftRef.current=next;setDraft(next);setDirty(true);};
 const open=(p:LandPlot)=>{selectedRef.current=p;setSelected(p);const d=structuredClone(p.draft||p.finished||initialLandDraft(p));draftRef.current=d;setDraft(d);setHistory([]);setFuture([]);editVersion.current=0;savedVersion.current=0;setDirty(false);setError('');requestId.current=crypto.randomUUID();setPhase('staging');};
 ...
 const beginConstruction=()=>{setPhase('construction');if(!selectedRef.current?.draft){editVersion.current++;setDirty(true);}};
 const finish=async()=>{if(await persist(true)){setPhase('exploring');setSelected(null);setDraft(null);}};
 const undo=()=>{if(!history.length||!draft)return;const prev=history[history.length-1];setHistory(h=>h.slice(0,-1));setFuture(f=>[draft,...f]);edit(prev,false);};
 const redo=()=>{if(!future.length||!draft)return;const next=future[0];setFuture(f=>f.slice(1));setHistory(h=>[...h,draft]);edit(next,false);};
 ...
 return {presetRevision,editPreset:(next:LandDraft)=>{setPresetRevision(n=>n+1);edit(next);},previewStatus,setPreviewStatus,previewRetry,retryPreview:()=>setPreviewRetry(n=>n+1),enabled,world,phase,selected,draft,error,saving,dirty,history,future,edit,open,close,purchase,beginConstruction,finish,undo,redo,reset,reopen,retry:()=>dirty?persist():load(),setPhase};
}
