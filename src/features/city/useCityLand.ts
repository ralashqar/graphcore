...
export function useCityLand(enabled:boolean,properties:CityProperty[],capacity:number,size:24|48){
 ...
 const [world,setWorld]=useState<LandWorld|null>(null),[phase,setPhase]=useState<LandPhase>('exploring'),[selected,setSelected]=useState<LandPlot|null>(null),[selectedVolume,setSelectedVolume]=useState<string|null>(null),[draft,setDraft]=useState<LandDraft|null>(null),[error,setError]=useState(''),[saving,setSaving]=useState(false),[dirty,setDirty]=useState(false),[history,setHistory]=useState<LandDraft[]>([]),[future,setFuture]=useState<LandDraft[]>([]);
 ...
 const edit=(next:LandDraft,record=true)=>{const p=selectedRef.current;if(!p)return;const previous=draftRef.current;next={...next,design:fitLandDesign(next.design,p.rotation)};if(next.sculpt?.version===4&&next.design.synarcKit){const oldAnchors=previous?.sculpt?.version===4?previous.sculpt.tileAnchors??[]:[],kept=new Set(next.sculpt.tileAnchors?.map(a=>a.id)),removed=new Set(oldAnchors.filter(a=>!kept.has(a.id)).map(a=>a.id));const kit={...next.design.synarcKit,paints:next.design.synarcKit.paints.filter(p=>!removed.has(p.id))};next={...next,design:{...next.design,synarcKit:reprojectSculptTiles(next.sculpt,kit,next.design)}};}if(record&&previous){setHistory(h=>[...h.slice(-49),previous]);setFuture([]);}editVersion.current++;draftRef.current=next;setDraft(next);setDirty(true);};
 const open=(p:LandPlot)=>{selectedRef.current=p;setSelected(p);setSelectedVolume(null);const d=structuredClone(p.draft||p.finished||initialLandDraft(p));draftRef.current=d;setDraft(d);setHistory([]);setFuture([]);editVersion.current=0;savedVersion.current=0;setDirty(false);setError('');requestId.current=crypto.randomUUID();setPhase('staging');};
