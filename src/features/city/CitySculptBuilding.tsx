...
import {CityStudioDetailBatches} from './CityStudioDetailBatches';
import {CityStudioTrimParts} from './CityStudioTrimParts';
import {studioFloorView,studioPieceFloor,subscribeStudioFloorView} from './cityStudioView';
...
import {prepareSculpt,type PreparedSculpt} from './citySculptService';
...
function clipTrianglesToStorey(vertices:number[],bottom:number,top:number){
 const output:number[]=[],clip=(polygon:number[][],limit:number,above:boolean)=>{const result:number[][]=[];for(let i=0;i<polygon.length;i++){const a=polygon[i],b=polygon[(i+1)%polygon.length],insideA=above?a[1]>=limit-.001:a[1]<=limit+.001,insideB=above?b[1]>=limit-.001:b[1]<=limit+.001;if(insideA)result.push(a);if(insideA!==insideB){const t=(limit-a[1])/(b[1]-a[1]);result.push([a[0]+(b[0]-a[0])*t,limit,a[2]+(b[2]-a[2])*t]);}}return result;};
 for(let i=0;i+8<vertices.length;i+=9){let polygon=[vertices.slice(i,i+3),vertices.slice(i+3,i+6),vertices.slice(i+6,i+9)];polygon=clip(polygon,bottom,true);if(polygon.length<3)continue;polygon=clip(polygon,top,false);for(let k=1;k+1<polygon.length;k++)output.push(...polygon[0],...polygon[k],...polygon[k+1]);}return output;
}
...
export function CitySculptBuilding({plot,draft,land}:{plot:LandPlot;draft:LandDraft;land?:CityLandController}){
 const {invalidate,gl,camera}=useThree();const property=useMemo(()=>landProperty(plot,draft),[plot.id,plot.rotation,draft.design,draft.color,draft.name]);
 const [nearInterior,setNearInterior]=useState(!!land),interiorCheck=useRef(0);
 useFrame((state)=>{if(land)return;if(state.clock.elapsedTime-interiorCheck.current<.5)return;interiorCheck.current=state.clock.elapsedTime;const p=landPosition(plot),near=Math.hypot(camera.position.x-p.x,camera.position.z-p.z)<60;setNearInterior(previous=>previous===near?previous:near);});
 const [cameraSector,setCameraSector]=useState(0),sectorCheck=useRef(0);
 useFrame((state)=>{if(!land||state.clock.elapsedTime-sectorCheck.current<.12)return;sectorCheck.current=state.clock.elapsedTime;const p=landPosition(plot),dx=camera.position.x-p.x,dz=camera.position.z-p.z,a=plot.rotation*Math.PI/2,x=dx*Math.cos(a)-dz*Math.sin(a),z=dx*Math.sin(a)+dz*Math.cos(a),sector=(Math.round(Math.atan2(x,z)/(Math.PI/4))+8)%8;setCameraSector(previous=>previous===sector?previous:sector);});
 const grounds=usePreparedCity(useMemo(()=>[property],[property]),true,true,true);
 const [result,setResult]=useState<PreparedSculpt|null>(null),[error,setError]=useState<string|null>(null),[pending,setPending]=useState(true);
 const [previewResult,setPreviewResult]=useState<PreparedSculpt|null>(null);
 ...
 const subscribeFloor=useCallback((listener:()=>void)=>subscribeStudioFloorView(plot.id,listener),[plot.id]);
 const floorSnapshot=useCallback(()=>studioFloorView(plot.id),[plot.id]);
 const floorView=useSyncExternalStore(subscribeFloor,floorSnapshot);
 ...
 const volumeMaterials=useMemo(()=>Object.fromEntries(((draft.sculpt?.version===4||draft.sculpt?.version===5||draft.sculpt?.version===6)?draft.sculpt.volumes:[]).map(volume=>{const wall=citySurfaceMaterial(false,volume.wallTexture??draft.design.textures?.wall),roof=citySurfaceMaterial(false,volume.roofTexture??draft.design.textures?.roof);wall.color.set(palette.wall);roof.color.set(draft.design.roof==='planted'?'#829a73':palette.roof);return [volume.id,{wall,roof}];})),[draft.sculpt,draft.design.textures?.wall,draft.design.textures?.roof,draft.design.roof,palette.wall,palette.roof]);
 ...
 const kitInfill=useMemo(()=>{const infill=shown?.kit?.infill??[];if(!sliced)return infill;return infill.filter(band=>{const level=band.floor??shown?.floors.findLast(f=>band.y>=f.bottom-.03)?.floor??0;if(floorView.mode==='floor')return level===floorView.floor;if(level>floorView.floor)return false;if(level<floorView.floor)return true;const sector=(Math.round(band.rotation/(Math.PI/4))+8)%8;return Math.cos((sector-cameraSector)*Math.PI/4)<=.35;});},[shown?.kit?.infill,shown?.floors,sliced,floorView.mode,floorView.floor,cameraSector]);
 const storeyBounds=shown?.floors[floorView.floor];
 const visibleCurvedGeometry=useMemo(()=>{if(!sliced||!shown?.curvedVertices||!storeyBounds)return curvedGeometry;const bottom=floorView.mode==='floor'?storeyBounds.bottom:(shown.floors[0]?.bottom??0),top=storeyBounds.top;return Object.fromEntries(Object.entries(shown.curvedVertices).map(([kind,vertices])=>{const g=new BufferGeometry();g.setAttribute('position',new Float32BufferAttribute(clipTrianglesToStorey(vertices,bottom,top),3));g.computeVertexNormals();return [kind,g];})) as typeof curvedGeometry;},[sliced,shown,floorView.mode,storeyBounds,curvedGeometry]);
 const visibleCurvedVolumeGeometry=useMemo(()=>{if(!sliced||!shown?.curvedVolumeWalls||!storeyBounds)return curvedVolumeGeometry;const bottom=floorView.mode==='floor'?storeyBounds.bottom:(shown.floors[0]?.bottom??0),top=storeyBounds.top;return Object.fromEntries(Object.entries(shown.curvedVolumeWalls).map(([id,vertices])=>{const g=new BufferGeometry();g.setAttribute('position',new Float32BufferAttribute(clipTrianglesToStorey(vertices,bottom,top),3));g.computeVertexNormals();return [id,g];})) as typeof curvedVolumeGeometry;},[sliced,shown,floorView.mode,storeyBounds,curvedVolumeGeometry]);
 useEffect(()=>()=>{if(visibleCurvedGeometry&&visibleCurvedGeometry!==curvedGeometry)Object.values(visibleCurvedGeometry).forEach(g=>g.dispose());if(visibleCurvedVolumeGeometry!==curvedVolumeGeometry)Object.values(visibleCurvedVolumeGeometry).forEach(g=>g.dispose());},[visibleCurvedGeometry,visibleCurvedVolumeGeometry,curvedGeometry,curvedVolumeGeometry]);
 const floorPieces=useMemo(()=>{if(!sliced||!shown?.studio?.interiorLevels)return null;const groups=new Map<string,{floor:number;sector:number;pieces:typeof shown.studio.pieces}>(),assemblies=displayedRecipe?.studio.assemblies??[];for(const piece of shown.studio.pieces){if(assemblies.some(a=>a.kind==='stair'&&(piece.id===a.id||piece.id.startsWith(a.id+'/'))))continue;const bay=shown.studio.bays.find(b=>piece.id===b.id||piece.id.startsWith(b.id+'/')||piece.id===`terrace/${b.id}`),floor=studioPieceFloor(piece,shown.studio.bays,shown.floors,assemblies),sector=bay?(Math.round(bay.rotation/(Math.PI/4))+8)%8:-1,key=`${floor}/${sector}`;let group=groups.get(key);if(!group){group={floor,sector,pieces:[]};groups.set(key,group);}group.pieces.push(piece);}return [...groups.values()];},[shown,sliced,displayedRecipe?.studio.assemblies]);
 // Floor slicing hides roof openings and the free faces above the cut (merged batches draw a subset).
 const hiddenDetail=useMemo(()=>{if(!sliced||!shown?.studio)return null;const out=new Set<string>((shown.studio.roofOpenings??[]).map(p=>`roof/${p.partId}`));for(const face of shown.studio.freeFaces??[]){const top=Math.max(...face.floors),away=Math.cos((((Math.round(face.rotation/(Math.PI/4))+8)%8)-cameraSector)*Math.PI/4)<=.35;if(!(floorView.mode==='cutaway'&&(top<floorView.floor||top===floorView.floor&&away)))out.add(face.id);}return out;},[sliced,shown,floorView.mode,floorView.floor,cameraSector]);
 return <><CityPreparedBuildings properties={grounds} layer="grounds" reduced/><group name="sculpt-building" position={[center.x,0,center.z]} rotation={[0,plot.rotation*Math.PI/2,0]} scale={scale}>{geometry&&(['wall','trim','roof','glass','door'] as const).filter(kind=>(!usingKit&&!shown?.studio)||kind==='roof'&&!shown?.studio?.roofPatches&&!sliced).map(kind=><mesh key={kind} geometry={geometry[kind]} material={materials[kind]}/>)}{Object.entries(volumeGeometry).map(([id,parts])=>(['wall','roof'] as const).filter(kind=>(!usingKit&&!shown?.studio)||kind==='roof'&&!sliced).map(kind=><mesh key={`${id}-${kind}`} geometry={parts[kind]} material={volumeMaterials[id]?.[kind]??materials[kind]}/>))}{usingKit&&visibleCurvedGeometry&&(['wall','trim','glass','door'] as const).map(kind=><mesh key={`curve-${kind}`} geometry={visibleCurvedGeometry[kind]} material={materials[kind]}/>)}{usingKit&&Object.entries(visibleCurvedVolumeGeometry).map(([id,part])=><mesh key={`curve-volume-${id}`} geometry={part} material={volumeMaterials[id]?.wall??materials.wall}/>)}{usingKit&&<><CitySynarcKitMeshes pack={synarcPack} placements={kitPlacements}/>{kitInfill.map((band,i)=><mesh key={`band-${i}`} position={[band.x,band.y,band.z]} rotation={[0,band.rotation,0]} material={materials.wall}><boxGeometry args={[band.width,band.height,band.depth]}/></mesh>)}</>}{!sliced&&shown?.studio?.roofPatches&&<CityRoofMeshes patches={shown.studio.roofPatches} edges={shown.studio.roofEdges??[]}/>}{shown?.studio&&(floorPieces&&land?floorPieces.map(group=><group key={group.floor+'-'+group.sector} visible={(!sliced||floorView.mode==='cutaway'&&group.floor<=floorView.floor||floorView.mode==='floor'&&group.floor===floorView.floor)&&(!sliced||floorView.mode==='floor'||group.sector<0||Math.cos((group.sector-cameraSector)*Math.PI/4)<=.35)}><CityStudioMeshes pieces={group.pieces} version={studioKitVersion}/></group>):<CityStudioMeshes pieces={shown.studio.pieces} version={studioKitVersion}/>)}{shown?.details&&<CityStudioDetailBatches details={shown.details} center={center} full={!!land} hidden={hiddenDetail} seeThrough={!interior||!!land||nearInterior}>{shown.studio?.freeFaces&&shown.studio.portals&&<CityStudioFreeDoorLeaves plotId={plot.id} portals={shown.studio.portals} faces={shown.studio.freeFaces} hidden={hiddenDetail}/>}{shown.studio?.freeFaces&&<CityStudioTrimParts faces={shown.studio.freeFaces} trims={shown.studio.freeTrims??displayedRecipe?.studio.freeTrims} groundHeight={draft.design.groundHeight} visible={!sliced}/>}</CityStudioDetailBatches>}{(land||nearInterior)&&shown?.studio?.interiorLevels&&interior&&<CityStudioInteriorMeshes plotId={plot.id} levels={shown.studio.interiorLevels} portals={shown.studio.portals??[]} decks={shown.studio.decks} view={land?floorView:{mode:'whole',floor:0}} finish={interior.floorFinish} wallColor={interior.wallColor}/>} {shown&&!shown.studio&&<SculptDetails details={usingKit?shown.decorations.filter(d=>d.kind==='planter'||d.kind==='bollard'):shown.decorations} entrance={entrance} groundHeight={draft.design.groundHeight}/>} {entrance&&<SculptSign name={draft.name} color={draft.color} x={entrance.x} y={shown!.floors[0].top-(hasCanopy ? .47 : .6)} z={entrance.z} angle={entrance.angle} depth={hasCanopy ? 1 : .045} maxWidth={hasCanopy ? 2.2 : 3.2}/>} {entrance&&approach&&<mesh position={[entrance.x/2,.31,(10.4+entrance.z)/2]} rotation={[0,approach.rotation,0]}><boxGeometry args={[1.8,.05,approach.length]}/><meshStandardMaterial color="#cbc7b5" roughness={1}/></mesh>}</group></>;
}
