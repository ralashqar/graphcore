...
import {pointInLoop,storeySpanForTop,storeyStartForBottom,verticalPlaneHeight} from './studioStoreys';
import {rotateStudioVolume,snapRotation,sweptAngle} from '../../domain/cityStudioRotate';
...
export function useStudioInteraction(options:StudioInteractionOptions){
 ...
 /** Tiny Glade-style stacking: drawing over an existing part starts on the storey above it. */
 const stackFloor=(r:StudioRecipe,x:number,y:number,fallback:number)=>{const o=current.current,origin=ray(x,y).origin;let best=fallback,distance=Infinity;for(const v of r.volumes){if(v.operation!=='add')continue;const next=v.startFloor+v.spanFloors;if(next>=8||next<=fallback)continue;const p=ground(x,y,sculptFloorBottom(next,o.draft.design.groundHeight,o.draft.design.upperHeight));if(!p)continue;const inside=v.kind==='ellipse'?((p.x-v.x)/(v.width/2))**2+((p.z-v.z)/(v.depth/2))**2<=1:v.kind==='polygon'&&v.vertices?pointInLoop(p.x,p.z,sculptPrimitiveBoundary(v)):Math.abs(p.x-v.x)<=v.width/2&&Math.abs(p.z-v.z)<=v.depth/2;const d=p.distanceTo(origin);if(inside&&d<distance){distance=d;best=next;}}return best;};
  ...
  const down=(e:PointerEvent)=>{
   ...
   const drawFloor=draw&&o.tool!=='cut'&&!e.altKey?stackFloor(r,e.clientX,e.clientY,o.floor):o.floor;
   const start=ground(e.clientX,e.clientY,handle&&chosen?sculptFloorBottom(chosen.startFloor,o.draft.design.groundHeight,o.draft.design.upperHeight):sculptFloorBottom(drawFloor,o.draft.design.groundHeight,o.draft.design.upperHeight));if(!start)return;
   ...
   const volume=draw?{id:crypto.randomUUID(),kind:o.tool==='round'||o.tool==='oval'?'ellipse' as const:'rectangle' as const,operation:o.tool==='cut'||e.altKey?'subtract' as const:'add' as const,x:snap(start.x),z:snap(start.z),width:4,depth:4,startFloor:drawFloor,spanFloors:1}:chosen;
  ...
  const move=(e:PointerEvent)=>{
   ...
   else if(g.handle==='rotate'){const turned=rotateStudioVolume(v,snapRotation(sweptAngle(v.x,v.z,[g.start.x,g.start.z],[p.x,p.z]),e.shiftKey));if(!turned){setIssue('Ovals cannot turn yet. Try a circle or a block.');return;}setIssue('');next=turned;}
