...
export function CitySculptControls({land,plot,draft,orbitEnabled,viewTopDown}:{land:CityLandController;plot:LandPlot;draft:LandDraft;orbitEnabled:(enabled:boolean)=>void;viewTopDown:()=>void}){
 ...
 const recipe=(draft.sculpt?.version===4||draft.sculpt?.version===5||draft.sculpt?.version===6)?undefined:draft.sculpt;const center=landPosition(plot),scale=plot.size/24;
 ...
 const dragRecipe=(p:NonNullable<typeof pointer.current>,next:SculptPrimitive)=>{
  ...
  if(!p.base&&p.tool==='courtyard'&&(revised.version!==4&&revised.version!==5&&revised.version!==6))for(const level of revised.levels.filter(l=>l.floor>active))revised=setSculptShapes(revised,level.floor,[...level.shapes,next]);
  return revised;
 };
