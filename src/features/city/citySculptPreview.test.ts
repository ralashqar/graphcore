...
const recipe:SculptRecipe={version:3,levels:[{floor:0,shapes:[{id:'main',kind:'rectangle',operation:'add',x:0,z:0,width:12,depth:12}]}],attachments:[]};
...
test('live Sculpt intent is plot-scoped and never changes an authored recipe',()=>{
 ...
 assert.equal(sculptPreviewSnapshot('preview-test-a').recipe?.levels[0].shapes[0].width,14);
