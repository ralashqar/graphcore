...
export function CityStudioFreeOpeningGhost({ghost}:{ghost:FreeOpeningGhost}){
 ...
 return <group position={[ghost.x,ghost.y,ghost.z]} rotation={[0,ghost.rotation,0]}>
  <mesh geometry={fill} raycast={()=>null} renderOrder={5}><meshBasicMaterial color={ghost.door?'#f0b979':'#fff1c9'} transparent opacity={.38} depthWrite={false}/></mesh>
  <lineSegments geometry={edge} raycast={()=>null} renderOrder={6}><lineBasicMaterial color="#fffaf0" transparent opacity={.95} depthTest={false}/></lineSegments>
 </group>;
}
