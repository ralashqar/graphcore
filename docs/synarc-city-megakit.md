...
Quaternius Downtown City MegaKit Source now supplies the selected road and building modules. The inspected local pack at `C:\Users\daruk\Projects\GraphCore\Assets\Downtown City MegaKit[Source]` contains 315 glTF models, compared with 153 in Standard, plus the editable `Blends/ModularCity.blend` and engine projects. Its included Source licence identifies the assets as CC0. The original download is unchanged.

The implementation vendors 39 selected models and 104 dependency files under `assets/city/megakit/source`, with the Source licence and SHA-256 inventory. Regenerate from this pack with `CITY_MEGAKIT_SOURCE` set to the path above and `node scripts/build-city-megakit.mjs`. The importer removes Godot `convcolonly` collision hulls before assembly; these are not visible geometry. Complete sample buildings and engine-specific interior shaders remain excluded from the browser runtime.
...
Each of six paid tiers has two deterministic orientations. Floor counts remain 1, 1, 2, 4, 7 and 10; complete L-shaped assemblies use 12/14-metre frontages, a six-metre wing width and an eight-metre return. Lower wings end at whole floors. Source brick/metal facades remain on tiers 0, 1, 3 and 5; white-brick/marble facades add variety on tiers 2 and 4. Native four-metre windows span pairs of two-metre bays, with matching plain end panels when needed. Both sides retain the original module proportions and three-metre floor height. No facade is scaled to fit.

Source door frames complete the entrances. Unbranded native-size awnings face the courtyard on tiers 1�3, leaving the outward hero billboards clear. Roof cornices match the facade family; sparse distant presets retain the footprint and approximate material palette. Whole buildings remain within the 16-metre envelope.

Straights now include native double-yellow centre lines and broken lane dividers; a separate shared arrow-pair mesh is instanced at two approach positions per block, not on every six-metre segment. Native curved markings follow perimeter bends. Existing junction bodies, crosswalks, curbs and connector coordinates remain in use. Decals sit slightly above asphalt to avoid coplanar flicker.

The build exports 24 building assets plus eleven infrastructure/prop assets. glTF Transform deduplicates, welds, conservatively simplifies with locked borders, and applies Meshopt compression with 16-bit positions. WebP textures are bounded to 1024 px; unused normal/ORM maps and vertex layers are removed because runtime materials use Lambert shading. The complete kit is embedded in one GLB; no decoder CDN is required. Quantized positions are expanded before node transforms are baked into runtime geometry.
...
## Source-pack upgrade verification (September 2026)

The Source pack replaces Standard as the vendor input. Only selected dependencies are committed; the user's original pack and Blender project remain untouched. The final runtime GLB is 7,402,488 bytes with 24 building presets, 17 materials and eight embedded textures. Source hash checks, glTF validation, collision-material exclusion, plot bounds and road-socket continuity pass. Billboard artwork, failure fallback, real sign selection, repeated right-drag navigation, central reset and reduced motion pass without runtime errors. Source-native facade widths are used consistently on both wings, including four-metre windows paired across the two-metre assembly grid.
