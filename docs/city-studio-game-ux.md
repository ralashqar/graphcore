...
The "stale eye-level wall" frame was not stale. The canvas was drawing the studio camera correctly, but that camera stood inside a neighbouring city building. The Front preset puts the camera 1.25 plot sizes in front of the plot, and the Orbit preset and free zoom (up to 3 plot sizes) can reach just as far. In the 48 m test world, the Front pose for the first test plot lands inside demo building `demo-48` on the next block. Its recipe finishes preparing in a worker some 3 to 30 s after the studio opens, so whether it was already drawn depended on timing. That is why the frame showed up in about half of the runs and seemed tied to large commits. It also explains why the reported camera never changed and why reloading the page cleared it (the building had not arrived yet). The studio now keeps its camera clear of neighbouring buildings (`src/domain/cityStudioCameraClearance.ts`). Each city property and each other test-world plot is treated as its whole protected plot footprint (plus 2 m), up to its recipe's massing top plus a roof allowance. Against the rendered demo city, this estimate sits 0.5 to 5 m above the real roofs. Preset and glide destinations keep their framing distance and rise over the neighbour, like a crane shot; for the first test plot, Front now looks down at about 30°. They come in along the sight line only if no elevation up to about 69° clears. After OrbitControls and any glide, every frame, the camera is also pulled in along its sight line to just before the first footprint it would enter, so free zooming and orbiting cannot enter a neighbour either. Targets panned over a neighbour are not trapped. Browser check: `scripts/city-studio-stale-camera-browser.mjs` (`CITY_REPEAT`, `CITY_LATE_MS`, `CITY_BACKEND=webgl`).
