...
def convert(source, rig):
    if source.get('space') != 'g1' or source['provenance']['adapter'] != 'g1-soma-1.0.0' or rig['id'] != 'humanoid.soma.v2':
    ...
    origins = {side+part:side.lower()+'_'+joint for side in ('Left','Right') for part,joint in [('Leg','hip_pitch_skel'),('Arm','shoulder_pitch_skel')]}
    offsets = {}
    for target, (child, source_child) in CHAINS.items():
        a = target_rest[child] - target_rest[target]
        b = source_rest[source_child] - source_rest[origins.get(target,MAPPING[target])]
        if min(a.length, b.length) < .001: raise ValueError('Degenerate anatomical calibration')
        offsets[target] = a.rotation_difference(b)
    source_leg = sum((source_rest[a]-source_rest[b]).length for a,b in [('left_hip_pitch_skel','left_knee_skel'),('left_knee_skel','left_ankle_roll_skel')])
    ...
    height_offset = target_rest['Hips'].y + min(j['rest'][1] for j in source['joints'])*scale
    frames = []
    for f in source['frames']:
        global_source, source_positions = [], []
        for j,q in zip(source['joints'], f['rotations']):
            parent=j['parent']
            global_source.append(global_source[parent] @ quat(q) if parent>=0 else quat(q))
            source_positions.append(source_positions[parent]+global_source[parent]@(Vector(j['rest'])-Vector(source['joints'][parent]['rest'])) if parent>=0 else Vector(f['root']))
        global_target, local = {}, []
        for j in rig['joints']:
            ...
            if name in MAPPING:
                ...
                if name in CHAINS:
                    child, source_child = CHAINS[name]
                    direction = source_positions[src[source_child]]-source_positions[src[origins.get(name,MAPPING[name])]]
                    current = desired @ (target_rest[child]-target_rest[name])
                    if min(current.length,direction.length)<.001: raise ValueError('Degenerate generated limb')
                    desired = current.rotation_difference(direction) @ desired
            else:
        ...
        root=[v*scale for v in f['root']];root[1]+=height_offset
        frames.append({'root':root, 'rotations':local})
    ...
    return {**source, 'space':'soma','joints':joints,'restRotations':[[0,0,0,1] for _ in joints],'frames':frames}, {
        'version':ADAPTER, 'rootScale':scale, 'heightOffset':height_offset, 'mappedJoints':list(MAPPING),
...
if __name__ == '__main__':
    ...
    if recipe.get('retargetRevision') != ADAPTER: raise ValueError('Frozen retarget revision mismatch')
