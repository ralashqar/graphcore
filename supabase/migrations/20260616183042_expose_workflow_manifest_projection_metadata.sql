...
begin
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'manifestPurpose', metadata ->> 'manifestPurpose',
          ...
          'lastProviderPollAt', metadata ->> 'lastProviderPollAt',
          'falRequestId', metadata ->> 'falRequestId',
          'activeChildRequestIds', coalesce(metadata -> 'activeChildRequestIds', outputs #> '{workflowRuntime,activeChildRequestIds}', outputs #> '{workflow_runtime,activeChildRequestIds}', outputs -> 'childRequests', outputs -> 'child_requests', '[]'::jsonb),
          'activeChildRunIds', coalesce(metadata -> 'activeChildRunIds', outputs #> '{workflowRuntime,activeChildRunIds}', outputs #> '{workflow_runtime,activeChildRunIds}', outputs -> 'childRunIds', outputs -> 'child_run_ids', '[]'::jsonb),
          'readyArtifactRoles', coalesce(metadata -> 'readyArtifactRoles', outputs #> '{workflowRuntime,readyArtifactRoles}', outputs #> '{workflow_runtime,readyArtifactRoles}', outputs -> 'readyArtifactRoles', outputs -> 'ready_artifact_roles', '[]'::jsonb),
          'readyArtifactKeys', coalesce(metadata -> 'readyArtifactKeys', outputs #> '{workflowRuntime,readyArtifactKeys}', outputs #> '{workflow_runtime,readyArtifactKeys}', outputs #> '{workflowRuntime,scopedAssetKeys}', outputs #> '{workflow_runtime,scopedAssetKeys}', outputs -> 'readyArtifactKeys', outputs -> 'ready_artifact_keys', '[]'::jsonb),
          'recoveryHints', coalesce(metadata -> 'recoveryHints', outputs #> '{workflowRuntime,recoveryHints}', outputs #> '{workflow_runtime,recoveryHints}', outputs -> 'recoveryHints', outputs -> 'recovery_hints', outputs -> 'diagnostics', '[]'::jsonb),
          'streaming', coalesce(metadata -> 'streaming', outputs #> '{workflowRuntime,streaming}', outputs #> '{workflow_runtime,streaming}', outputs -> 'streaming', '{}'::jsonb),
          'streamingStatus', coalesce(metadata ->> 'streamingStatus', outputs #>> '{workflowRuntime,streamingStatus}', outputs #>> '{workflow_runtime,streamingStatus}', outputs #>> '{streaming,status}'),
          'streamingEventCount', coalesce(nullif(metadata ->> 'streamingEventCount', '')::integer, nullif(outputs #>> '{workflowRuntime,streamingEventCount}', '')::integer, nullif(outputs #>> '{workflow_runtime,streamingEventCount}', '')::integer, nullif(outputs #>> '{streaming,eventCount}', '')::integer, 0),
          'streamingPartialArtifactKeys', coalesce(metadata -> 'streamingPartialArtifactKeys', outputs #> '{workflowRuntime,streamingPartialArtifactKeys}', outputs #> '{workflow_runtime,streamingPartialArtifactKeys}', outputs #> '{streaming,partialArtifactKeys}', '[]'::jsonb),
          'staleRequestRestarted', coalesce((metadata ->> 'staleRequestRestarted')::boolean, false),
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'activeNodes', coalesce(active_nodes, '[]'::jsonb),
    ...
    jsonb_build_object(
      ...
      'artifactsUpdatedAt', artifacts_updated_at,
      'activeNodes', coalesce(active_nodes, '[]'::jsonb),
      'workflowRuntime', jsonb_build_object(
        'workflowFamily', coalesce(request_row.metadata ->> 'workflowFamily', run_row.metadata ->> 'workflowFamily', request_row.metadata ->> 'screenplayAnimaticRole', request_row.metadata ->> 'sequenceAnimaticRole', ''),
        'workflowCommandAction', coalesce(request_row.metadata ->> 'workflowCommandAction', run_row.metadata ->> 'workflowCommandAction', request_row.metadata #>> '{command,action}', run_row.input #>> '{sceneBoardCommand,action}', ''),
        'activeManifestPurpose', coalesce(active_nodes #>> '{0,manifestPurpose}', ''),
        'activeProgressLabel', coalesce(active_nodes #>> '{0,progressLabel}', active_node_label, ''),
        'activeChildRequestIds', coalesce(active_nodes #> '{0,activeChildRequestIds}', '[]'::jsonb),
        'activeChildRunIds', coalesce(active_nodes #> '{0,activeChildRunIds}', '[]'::jsonb),
        'providerStatus', active_nodes #>> '{0,providerStatus}',
        'providerRequestId', coalesce(active_nodes #>> '{0,providerRequestId}', active_nodes #>> '{0,falRequestId}'),
        'streaming', coalesce(active_nodes #> '{0,streaming}', '{}'::jsonb),
        'streamingStatus', active_nodes #>> '{0,streamingStatus}',
        'streamingEventCount', coalesce(nullif(active_nodes #>> '{0,streamingEventCount}', '')::integer, 0),
        'streamingPartialArtifactKeys', coalesce(active_nodes #> '{0,streamingPartialArtifactKeys}', '[]'::jsonb),
        'failedNodeKey', case when latest_step_error is not null then active_node_key else null end,
        'failedNodePurpose', case when latest_step_error is not null then coalesce(active_nodes #>> '{0,manifestPurpose}', '') else null end,
        'readyArtifactCount', coalesce(array_length(artifact_keys, 1), 0),
        'scopedAssetKeys', coalesce(to_jsonb(preview_asset_keys), '[]'::jsonb),
        'recoveryHints', coalesce(active_nodes #> '{0,recoveryHints}', '[]'::jsonb)
      )
    ),
