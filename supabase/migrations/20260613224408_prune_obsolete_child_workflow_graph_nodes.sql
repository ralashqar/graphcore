...
begin
  ...
  delete from public.output_workflow_edges e
  where e.workflow_id = ensured_workflow.id
    and not exists (
      select 1
      from jsonb_array_elements(coalesce(p_edges, '[]'::jsonb)) as edge_value(value)
      where edge_value.value ->> 'key' = e.key
    );

  delete from public.output_workflow_nodes n
  where n.workflow_id = ensured_workflow.id
    and not exists (
      select 1
      from jsonb_array_elements(coalesce(p_nodes, '[]'::jsonb)) as node_value(value)
      where node_value.value ->> 'key' = n.key
    );
