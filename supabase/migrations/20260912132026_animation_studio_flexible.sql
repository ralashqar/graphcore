-- Version 3 graphs reuse the existing RLS and fenced command boundary.
create or replace function public.animation_studio_command(p_actor uuid,p_command jsonb,p_prepared jsonb default '{}') returns jsonb
...
begin
 ...
 if act in ('save','restore','apply_edit') then
  if coalesce(p_prepared->'graph'->>'version','') not in ('2','3') then raise exception 'Validated graph required'; end if;
  ...
  insert into public.game_jobs(id,draft_id,animation_workspace_id,kind,input,requested_by,reserved_credits) values(jid,did,wid,'generate',jsonb_build_object('projectId',pid,'sourceRevision',w.revision,'studioPlan',p_command,'graph',w.graph,'studioHistory',coalesce((select jsonb_agg(h.turn) from (select jsonb_build_object('prompt',input->'studioPlan'->>'prompt','summary',checkpoint->'studioEdit'->>'summary') as turn from public.game_jobs where animation_workspace_id=wid and input->'studioPlan' is not null order by created_at desc limit 12) h),'[]'::jsonb),'context',jsonb_build_object('studioVersion','animation-studio-1.0.0')),p_actor,reserve);
  ...
  for item in select * from jsonb_array_elements(w.graph->'nodes') where w.graph->>'version'='2' or value->>'kind'='clip' loop
  ...
  if coalesce(p_prepared->'snapshot'->'sourceGraph',p_prepared->'snapshot'->'graph') is distinct from w.graph then raise exception 'Frozen graph mismatch'; end if;
 ...
 if act in ('save','restore','apply_edit') or (act='review_clip' and p_command->>'decision'='accepted') then
...
create function public.animation_studio_finish_edit(p_job uuid,p_worker text,p_fence bigint,p_result jsonb) returns boolean
language plpgsql security invoker set search_path=public,pg_temp as $$
declare j public.game_jobs; w public.animation_studio_workspaces; v_phase text; item jsonb;
begin
 select * into j from public.game_jobs where id=p_job;
 if j.animation_workspace_id is null then raise exception 'Studio job required'; end if;
 perform pg_advisory_xact_lock(hashtextextended('animation-studio:'||j.animation_workspace_id,0));
 select * into w from public.animation_studio_workspaces where id=j.animation_workspace_id for update;
 select * into j from public.game_jobs where id=p_job for update;
 if j.status<>'running' or j.lease_owner<>p_worker or j.fence<>p_fence or j.lease_until<=now() then return false; end if;
 if p_result->'graph'->>'version' is distinct from '3' then raise exception 'Validated flexible graph required'; end if;
 ...
 if not app_private.can_edit_draft(w.draft_id) then v_phase:='edit.permission_changed';
 elsif w.revision<>(j.input->>'sourceRevision')::integer then v_phase:='edit.conflict';
 elsif jsonb_array_length(p_result->'scopeExpansion')>0 then v_phase:='edit.scope_review';
 else
  for item in select * from jsonb_array_elements(p_result->'graph'->'nodes') where value->>'clipId' is not null loop
   if not exists(select 1 from public.game_animation_candidates c join public.game_animation_reviews r on r.candidate_id=c.id where c.id=(item->>'clipId')::uuid and c.draft_id=w.draft_id and r.decision='accepted' and c.clip->>'motionContract'=item->>'contractHash') then raise exception 'Unreviewed clip in edit'; end if;
  end loop;
  update public.animation_studio_workspaces set graph=p_result->'graph',revision=revision+1,updated_at=now() where id=w.id returning * into w;
  insert into public.animation_studio_revisions(workspace_id,draft_id,revision,graph,created_by) values(w.id,w.draft_id,w.revision,w.graph,j.requested_by);
  v_phase:='edit.applied';
 end if;
 update public.game_jobs set status='completed',phase=v_phase,checkpoint=checkpoint||jsonb_build_object('studioGraph',p_result->'graph','studioEdit',p_result,'appliedRevision',case when v_phase='edit.applied' then w.revision else null end),lease_until=null,updated_at=now() where id=j.id;
 perform public.game_settle_reservation(j.id,j.reserved_credits);
 return true;
end $$;
revoke all on function public.animation_studio_finish_edit(uuid,text,bigint,jsonb) from public,anon,authenticated;
grant execute on function public.animation_studio_finish_edit(uuid,text,bigint,jsonb) to service_role;
