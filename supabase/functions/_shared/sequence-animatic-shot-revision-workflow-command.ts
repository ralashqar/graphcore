...
export async function runSequenceAnimaticShotRevisionWorkflowCommand(input: {
  client: {
  ...
  payload: unknown
    ...
    const payload = sequenceAnimaticShotRevisionWorkflowEnsureRequestSchema.parse(input.payload)
    ...
    if (existing?.workflowId) {
      return sequenceAnimaticShotRevisionWorkflowEnsureResponseSchema.parse({
        ok: true,
        masterRequest,
    ...
    const graphResult = buildValidatedSequenceAnimaticTemplateGraph({
      ...
      templateKey: sequenceAnimaticShotRevisionTemplateKey,
      rawInput: {
        workflowId,
    ...
    const workflowPayload = {
      ...
      metadata: {
        ...commonConfig,
        readyToRun: true,
        workflowTemplateKey: sequenceAnimaticShotRevisionTemplateKey,
    ...
    const ensured = await ensureMappedChildWorkflow({
      ...
      parentRequestId: blockRequest.id,
      role: 'shot_revision',
      identityKey: 'revisionId',
      identityValue: revisionId,
      workflow: workflowPayload,
      nodes,
      edges,
      request: requestPayload,
    })
    const revisionRequest = ensured.request
