...
export function cinematicScriptPlannerSystemPrompt(
  ...
  return [
    ...
    'Absolute shot startSeconds/endSeconds are computed by GraphCore after compilation. Do not try to author absolute timeline offsets.',
    'Dialogue, action, and audio beat timings may be relative within a shot. Keep timeline timing local to the shot rather than global to the full sequence.',
    'When a shot includes dialogue, actions, audio, or custom beats, provide local startSeconds/endSeconds for each beat whenever the ordering is inferable.',
    'Use 0 as shot start and durationSeconds as shot end when reasoning about local timing.',
    'For multiple dialogue, action, or audio beats in one shot, stagger their windows in order across the shot instead of leaving every beat untimed or full-shot.',
    'Do not leave all cue timing blank unless the cue genuinely spans the entire shot and there is only one such cue.',
...
export function cinematicShotSkeletonPlannerSystemPrompt(
  ...
  return [
    ...
    'Absolute shot startSeconds/endSeconds are computed later by GraphCore. Do not plan global timeline offsets in this pass.',
...
export function cinematicShotAuthorshipSystemPrompt(
  ...
  return [
    ...
    'Absolute shot startSeconds/endSeconds are computed by GraphCore after compilation. Keep any timing local to the shot.',
    'For dialogue, action, and audio beats, include local startSeconds/endSeconds by default unless the cue truly spans the full shot.',
    'Use 0 as shot start and durationSeconds as shot end when reasoning about local timing, and keep all beat windows inside that range.',
    'If a shot contains multiple dialogue lines, actions, or audio cues, stagger them in order across the shot instead of leaving every beat untimed or full-shot.',
