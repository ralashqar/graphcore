...
export async function animationCommand(admin: any, actor: string, raw: unknown) {
  ...
  if (!prior.data && command.action === 'generate_animation') {
    const prepared=await prepareAnimationRequest(admin,actor,command);rig=prepared.rig;provider=prepared.provider
  }
...
export async function prepareAnimationRequest(admin:any,actor:string,command:{recipe:import('../../../src/domain/game/v3/animation.ts').MotionRecipe;projectId:string;draftId:string;expectedRevision:number}) {
  let rig:any=null,provider:any=null
    ...
    if(command.recipe.state==='vault')throw new HttpError(400,'Vault uses a procedural approximation; generated replacement is not validated')
    if (!rig) throw new HttpError(400, 'Only the supported humanoid mannequin is admitted')
    if (rig.id === 'humanoid.fabric-ybot.v1' && !(command.recipe.version===2&&command.recipe.retargetRevision==='g1-humanoid-1.2.0'||command.recipe.version===1&&command.recipe.retargetRevision==='soma-fabric-1.0.0')) throw new HttpError(400, 'Fabric mannequin requires the coordinated humanoid adapter')
