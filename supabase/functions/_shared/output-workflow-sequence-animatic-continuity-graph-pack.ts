...
export async function sequenceAnimaticContinuityInput(
  ...
  const animaticReferenceCatalog = configuredCatalog.length > 0
    ...
    : helpers.readArray(helpers.sequenceAnimaticReferenceCatalog({
...
export async function sequenceAnimaticContinuitySeedGraph(
  ...
  const continuityGraphV2 = helpers.sequenceAnimaticEmptyGraphV2(continuityPlannerContext)
  const outputs = {
    continuityGraphV2,
...
export async function sequenceAnimaticContinuityGlobalPlan(
  ...
  const graph = helpers.parseSequenceAnimaticGraphV2(helpers.readPreferredUpstreamRecord(context.upstream, ['continuity_seed_graph'], ['continuityGraphV2', 'continuity_graph_v2']))
  const storyboardBlock = helpers.sequenceAnimaticGlobalStoryboardBlock(continuityPlannerContext)
  let planned: Awaited<ReturnType<SequenceAnimaticWorkflowNodePackHelpers['runBackgroundStructuredNode']>>
  try {
    const allShots = helpers.sequenceAnimaticBlockShots(continuityPlannerContext, storyboardBlock)
    planned = await helpers.runBackgroundStructuredNode({
      ...
      schema: helpers.sequenceAnimaticContinuityBlockDeltaSchema,
      ...
      fallback: helpers.emptySequenceAnimaticContinuityBlockDelta('global', 'LLM global continuity planner did not produce valid structured output.'),
  ...
  const continuityBlockDelta = helpers.repairSequenceAnimaticContinuityBlockDelta({
    delta: helpers.parseSequenceAnimaticContinuityBlockDelta({
...
export async function sequenceAnimaticContinuityGlobalMerge(
  ...
  const delta = helpers.repairSequenceAnimaticContinuityBlockDelta({
    delta: helpers.parseSequenceAnimaticContinuityBlockDelta(deltaRecord),
    graph,
    continuityPlannerContext,
    storyboardBlock,
    allowDeterministicFallback: false,
  })
  const continuityGraphV2 = helpers.mergeSequenceAnimaticContinuityGraphV2({ graph, delta, continuityPlannerContext })
  const coverage = helpers.sequenceAnimaticContinuityCoverage(continuityGraphV2, continuityPlannerContext)
...
export async function sequenceAnimaticContinuityBlockPlan(
  ...
  const config = helpers.asRecord(context.node.config)
  const continuityPlannerContext = helpers.readFirstUpstreamRecord(context.upstream, ['continuityPlannerContext', 'continuity_planner_context'])
  const graph = helpers.parseSequenceAnimaticGraphV2(helpers.readFirstUpstreamRecord(context.upstream, ['continuityGraphV2', 'continuity_graph_v2']))
  const storyboardBlock = helpers.asRecord(config.storyboardBlock)
  let planned: Awaited<ReturnType<SequenceAnimaticWorkflowNodePackHelpers['runBackgroundStructuredNode']>>
  try {
    const blockShots = helpers.sequenceAnimaticBlockShots(continuityPlannerContext, storyboardBlock)
    planned = await helpers.runBackgroundStructuredNode({
      nodeKey: context.node.key,
      ...
      fallback: helpers.emptySequenceAnimaticContinuityBlockDelta(helpers.readText(storyboardBlock.id) || context.node.key, 'LLM block continuity planner did not produce valid structured output.'),
...
export async function sequenceAnimaticContinuityBlockMerge(
  ...
  const config = helpers.asRecord(context.node.config)
  const blockSuffix = helpers.continuityBlockNodeSuffix(context.node.key)
  const continuityPlannerContext = helpers.readPreferredUpstreamRecord(context.upstream, ['continuity_input'], ['continuityPlannerContext', 'continuity_planner_context'])
  const graph = helpers.parseSequenceAnimaticGraphV2(helpers.readPreferredUpstreamRecord(context.upstream, helpers.previousContinuityGraphNodeKeys(blockSuffix), ['continuityGraphV2', 'continuity_graph_v2']))
...
export async function sequenceAnimaticContinuityGraphFinalize(
  ...
  const continuityGraphV2 = helpers.parseSequenceAnimaticGraphV2(helpers.readFirstUpstreamRecord(context.upstream, ['continuityGraphV2', 'continuity_graph_v2']))
  const finalized = helpers.finalizeSequenceAnimaticContinuityGraphV2(continuityGraphV2)
  const outputs = {
    ...finalized,
...
export async function sequenceAnimaticContinuityStructureArtifact(
  ...
  const continuityGraphV2 = helpers.parseSequenceAnimaticGraphV2(helpers.readPreferredUpstreamRecord(context.upstream, preferredGraphNodeKeys, ['continuityGraphV2', 'continuity_graph_v2']))
  ...
  const blockStates = isGlobalStructure
    ? helpers.sequenceAnimaticSeededBlockStatesFromCoverage(continuityGraphV2, continuityPlannerContext, previousBlockStates)
    : {
      ...previousBlockStates,
      ...helpers.sequenceAnimaticContinuityBlockStatesFromGraph(continuityGraphV2, {
  ...
  const pendingDeltas = Object.keys(continuityBlockDelta).length > 0 && helpers.readText(config.storyboardBlockId) && !isGlobalStructure
    ...
    : {}
  const coverage = helpers.sequenceAnimaticContinuityCoverage(continuityGraphV2, continuityPlannerContext, blockStates)
  ...
  const packBase = {
    ...
    continuityGraphStatus: Number(coverage.totalShots ?? 0) > 0 && helpers.readStringArray(coverage.missingShotIds).length === 0
      ? 'ready'
      : helpers.sequenceAnimaticContinuityGraphStatusFromBlockStates(blockStates),
  ...
  const packWithAssets = helpers.withSequenceAnimaticContinuityAssetState(packBase, continuityGraphV2)
...
export async function sequenceAnimaticContinuityArtifact(
  ...
  const packBase = {
    ...
    blockStates,
    pendingDeltas,
    continuityGraphStatus: helpers.sequenceAnimaticContinuityGraphStatusFromBlockStates(blockStates),
    rejectedCandidates,
    plannerWarnings,
    plannerDiagnostics,
    anchorAssets,
