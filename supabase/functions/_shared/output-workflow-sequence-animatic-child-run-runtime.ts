...
export type SequenceAnimaticChildRunRuntimeHelpers = {
  ...
  loadSpatialViewpointMarkerMetadata: (input: {
    projectId: string
    draftId: string
  }) => Promise<LooseRecord[]>
...
function spatialViewpointEntityFromMarker(markerInput: unknown, helpers: SequenceAnimaticChildRunRuntimeHelpers) {
  const marker = helpers.asRecord(markerInput)
  const assetKey = helpers.readText(marker.screenshot_asset_key ?? marker.screenshotAssetKey)
  if (!assetKey) return null
  return {
    key: `spatial_viewpoint:${helpers.readText(marker.key) || helpers.readText(marker.id)}`,
    name: helpers.readText(marker.name) || 'Spatial viewpoint',
    type: 'location_spot',
    role: 'spatial_viewpoint_reference',
    summary: helpers.readText(marker.description) || 'User-authored camera viewpoint from the generated spatial world.',
    ...
    selectedReferenceVariantLabel: helpers.readText(marker.name) || 'Spatial viewpoint',
    selectedReferenceVariantType: 'spatial_viewpoint',
    referenceSelectionReason: 'Explicit user-authored spatial scouting viewpoint.',
    spatialViewpoint: {
      markerId: helpers.readText(marker.id),
      transform: helpers.asRecord(marker.transform),
      camera: helpers.asRecord(marker.camera),
      linkedEntityKey: helpers.readText(marker.linked_entity_key ?? marker.linkedEntityKey),
      linkedLocationKey: helpers.readText(marker.linked_location_key ?? marker.linkedLocationKey),
      linkedSceneId: helpers.readText(marker.linked_scene_id ?? marker.linkedSceneId),
      linkedSpotId: helpers.readText(marker.linked_spot_id ?? marker.linkedSpotId),
      linkedCoverageSetupId: helpers.readText(marker.linked_coverage_setup_id ?? marker.linkedCoverageSetupId),
    },
  }
}
...
async function augmentStoryboardBlockWorkflowAssetPackWithContinuityAssets(input: {
  ...
  const spatialViewpointEntities = (await helpers.loadSpatialViewpointMarkerMetadata({
    projectId: input.request.projectId,
    draftId: input.request.draftId,
  }))
    .map((marker) => spatialViewpointEntityFromMarker(marker, helpers))
    .filter((entry): entry is LooseRecord => Boolean(entry))
  ...
  const nextAssetPack = {
    ...
    spatialViewpointReferenceAssetKeys: [...new Set([
      ...helpers.readStringArray(assetPack.spatialViewpointReferenceAssetKeys),
      ...spatialViewpointEntities.map((entity) => helpers.readText(entity.primaryAssetKey)).filter(Boolean),
    ])],
  }
