...
export function buildSequenceAnimaticShotProductionWorkflowGraph(input: {
  ...
  const includeCoverageSubgraph = Boolean(readText(input.commonConfig.coverageSetupId) || readText(input.commonConfig.coverageAnchorScopeKey))
...
export function buildSequenceAnimaticZoneCoverageBoardWorkflowGraph(input: {
  ...
  const nodes = [
    ...
    sequenceAnimaticWorkflowNode(input.workflowId, input.draftId, 'zone_coverage_board_image', 'image_generation', 'Zone Camera Grid Image', 920, 120, {
      ...
      imageSize: { width: 2304, height: 1296 },
      planningOnly: true,
      planning_only: true,
